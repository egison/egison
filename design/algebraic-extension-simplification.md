...
> **状態 (2026-07-07): 解決済み。** 本文書が提起した代数拡大体上の零判定は、
> グレブナー基底エンジン (`polyNF`/`idealNF`、[cas-simplification.md](./cas-simplification.md) §3.3/G2)
> と主枝正規化 (§3.7/G7 — 負の定数 radicand を i·√(−x) に正規化し、ペア融合を主枝で健全化)
> で解決した。下の具体例そのもの (`5th-root-of-unity.egi` の `z1'^5 = 1`) は、
> 根号原子のペア積関係式を比較点で一度だけ `idealNF` 適用することでサンプル本体の
> assert として成立している (発火点原則: 重い関係式は演算経路に入れない)。
> 本文書は課題の定式化と当時の分析の歴史的記録として残す。
