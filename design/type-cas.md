...
data Monomial = Monomial
  { monoConsts  :: [(SymbolExpr, Integer)]  -- ConstFactor の冪の積（自由シンボルなしの Apply1/Quote）
  , monoSymbols :: [(SymbolExpr, Integer)]  -- Symbol の冪の積（平のシンボルのみ）
  , monoComps   :: [(SymbolExpr, Integer)]  -- CompFactor の冪の積（自由シンボルを含む Apply1/Quote）
  }
-- 冪指数は負も可（ローラン多項式のため）
```
...
| 分類 | 例 | 内部表現 | 性質 |
|----|----|----|----|
| **`ConstFactor`**（定数 Factor） | `'sqrt 2`, `'log 3`, `'sin (1+1)` | 自由シンボルなしの `Apply1`/`Quote` | 任意のシンボル `s` で微分すると 0（連鎖律対象が存在しない） |
| **`Symbol`**（平のシンボル） | `x`, `y`, `r`, `θ` | `CASFactor (Symbol _)` | `s` 自身で微分すると 1、それ以外は 0 |
| **`CompFactor`**（合成 Factor） | `'sin x`, `'sqrt x`, `'log (x+1)` | 自由シンボルを含む `Apply1`/`Quote` | 連鎖律で新しい Factor が生成されうる（微分で型構造が変わる可能性） |
...
`ConstFactor`, `Symbol`, `CompFactor` の3つは互いに disjoint で、`Factor` はこれらの和に等しい。

### 3分類の動機

`Poly` の原子集合スロットを 3 つに分けるのは、**`Poly c [cs] [ss] [fs]` が微分で閉じる十分条件** を型レベルで表現するためである:

> `fs = []` のとき、`Poly c [cs] [ss] []` は任意の `Symbol` での `∂/∂` で型保存になる。

これは:
- `cs` の各定数 Factor は微分で 0 になる（項が消えるか定数因子として残る）
- `ss` の各平のシンボルは標準的な冪則で微分される（最大でも `Poly c [cs] [ss] []` 内に閉じる）
- `fs` が空なので連鎖律対象が存在せず、新しい原子が生成されない

`fs ≠ []` の場合は連鎖律で新しい原子（例: `'cos x`）が生成されうるため、型保存は保証されない。詳細は後述の「微分演算子 `∂/∂`」セクションを参照。
...
x               : Symbol      -- 平のシンボル
y               : Symbol      -- 平のシンボル
'sqrt 2         : ConstFactor -- 引数 2 に自由シンボルなし
'log 3          : ConstFactor -- 引数 3 に自由シンボルなし
'sin (1 + 1)    : ConstFactor -- 引数 1+1 に自由シンボルなし
'sin x          : CompFactor  -- 引数 x に自由シンボル x が現れる
'sqrt x         : CompFactor  -- 引数 x に自由シンボル x が現れる
'log (x + 1)    : CompFactor  -- 引数 x+1 に自由シンボル x が現れる
```

分類のアルゴリズム:

1. `'f a1 a2 ...` の形の Factor について、各引数 `ai` の自由シンボル集合 `freeSymbols(ai)` を再帰的に計算
2. 全引数が `freeSymbols = ∅` ならば `ConstFactor`、それ以外は `CompFactor`
...
```
ConstFactor → Poly Integer [cs] [] []     -- cs に入る
Symbol      → Poly Integer [] [ss] []     -- ss に入る
CompFactor  → Poly Integer [] [] [fs]     -- fs に入る
```

なお `declare symbol 'sin x` のような合成原子の宣言構文は、簡約系に登録するためのものであり、結果の値は通常通り `CompFactor` 型である（`declare symbol` 構文の対象と `Symbol` 型の外延は一致しない）。

### クォート演算子 `'`

`'` は関数適用を停止して `ConstFactor` または `CompFactor` の値を生成する組み込み演算子。生成される値の型は引数の自由シンボル有無で決まる。

```egison
'sqrt 2         : ConstFactor     -- √2 を表す原子（引数 2 に自由シンボルなし）
'sin x          : CompFactor      -- sin(x) を表す原子（引数 x に自由シンボルあり）
'sin ('sqrt 2)  : ConstFactor     -- sin(√2) を表す原子（引数 'sqrt 2 にも自由シンボルなし）
```
...
```egison
def s := 'sqrt 2                -- s : ConstFactor

-- 開いた Poly への昇格
s + 1                           -- Poly Integer [..] [..] [..]

-- 閉じた Poly への昇格（型注釈あり）
def t : Poly Integer ['sqrt 2] [] [] := s + 1    -- ConstFactor は cs スロットに入る
```
...
sqrt n =
  ...
  else embed ('sqrt n)                            -- ConstFactor → Poly Integer [..] [..] [..] の embed
```

- `declare rule auto ('sqrt $x)^2 = x` はトップレベル宣言で、任意の `x` に対して `('sqrt x)^2` を `x` に簡約する
- 完全平方数のときは整数を開いた `Poly` に embed して返す
- そうでないとき、`'sqrt n` を生成し `Poly` に embed して返す。`n` は整数なので結果は `ConstFactor`
- `sin`, `cos` などの超越関数も同様にユーザーが定義できる

#### 呼び出し側での型の絞り込み

`sqrt` は開いた `Poly` を返すが、呼び出し側で型注釈を付けることで具体的な型に絞り込める。型チェッカーが `coerce` を自動挿入し、ランタイムで検証される。
...
```
ConstFactor  ⊂  Factor       -- 定数 Factor は Factor の真の部分型
Symbol       ⊂  Factor       -- 平のシンボルは Factor の真の部分型
CompFactor   ⊂  Factor       -- 合成 Factor は Factor の真の部分型

-- 各分類は対応するスロットに入る形で Poly に埋め込まれる
ConstFactor  ⊂  Poly Integer [..] [] []
Symbol       ⊂  Poly Integer [] [..] []
CompFactor   ⊂  Poly Integer [] [] [..]
Factor       ⊂  Poly Integer [..] [..] [..]
...
Integer            ⊂  MathValue
ConstFactor        ⊂  MathValue
Symbol             ⊂  MathValue
CompFactor         ⊂  MathValue
Factor             ⊂  MathValue
...
instance Embed ConstFactor (Poly Integer [..] [] []) where
...
instance Embed CompFactor (Poly Integer [] [] [..]) where
  embed f = ...  -- CASFactor sym → CASPoly [CASTerm (CASInteger 1) <[] | [] | [(sym, 1)]>]
...
Poly a [cs₁] [ss₁] [fs₁] → Poly a [cs₂] [ss₂] [fs₂]  (cs₁ ⊆ cs₂ ∧ ss₁ ⊆ ss₂ ∧ fs₁ ⊆ fs₂):
  内部表現は変わらない（Monomial の各スロットの原子は拡大後の集合の部分として有効）
```
...
| `Poly a [..] [..] [..] → ConstFactor` | 単一項で係数が1、`monoConsts` が単一原子の1乗、他スロットが空 |
| `Poly a [..] [..] [..] → Symbol` | 単一項で係数が1、`monoSymbols` が単一原子の1乗、他スロットが空 |
| `Poly a [..] [..] [..] → CompFactor` | 単一項で係数が1、`monoComps` が単一原子の1乗、他スロットが空 |
| `Frac a → a` | 分母が1か |
...
> 各スロットの和集合は **スロット内の原子集合に対して** 計算する。スロット間で原子は混ざらない（`Symbol` と `ConstFactor` を同じスロットに入れることはない）。
...
理由:
- `cs` の各定数 Factor (`'sqrt 2`, `'log 3` 等) は内部に自由シンボルを持たないため、任意の `s` で微分すると 0（連鎖律で対象がない）。微分結果に新しい原子は出ない
- `ss` の各平のシンボルは標準的な冪則で微分される: `∂/∂ x^n s = n * x^(n-1)` (if `x = s`) または `0` (if `x ≠ s`)。どちらも `Poly c [cs] [ss] []` 内に閉じる
- `fs = []` なので連鎖律対象が存在せず、`'cos x` のような新しい原子は生成されない

`fs ≠ []` の場合は連鎖律で新しい原子が生成されうるため、型保存ではない。例えば `∂/∂ (sin x : Poly Integer [] [] [('sin x)]) x = 'cos x` の結果型は `Poly Integer [] [] [('cos x)]` であり、`fs` スロットが変化する。

#### 型クラス定義

```egison
class Differentiable a where
  ∂/∂ : a -> Symbol -> a    -- 型保存（closed）なシグネチャ
```
...
constFactor : Matcher ConstFactor
symbol      : Matcher Symbol
compFactor  : Matcher CompFactor
...
`factor` は `Factor` 型全体のマッチャーで、`constFactor`, `symbol`, `compFactor` の和集合として振る舞う。各分類専用のマッチャー（`constFactor`, `symbol`, `compFactor`）も提供し、`term m` のモノミアル分解では各スロット用に使い分ける。
...
term m  の  ($c, ($mc, $ms, $mf))
  → c  : a
    mc : [(ConstFactor, Integer)]
    ms : [(Symbol, Integer)]
    mf : [(CompFactor, Integer)]
```
...
-- :+ は項（Term Integer [] [x] []）と残りに分解
match expr as poly integer with
  | $a :+ $rest -> ...   -- a : Term Integer [] [x] [], rest : Poly Integer [] [x] []

-- 項をさらに係数と3スロットモノミアルに分解
match expr as poly integer with
  | $a :+ _ ->
    match a as term integer with
      | ($c, ($mc, $ms, $mf)) -> ...
      -- c  : Integer
      -- mc : [(ConstFactor, Integer)]   -- このスロットは空（[] [x] [] なので）
      -- ms : [(Symbol, Integer)]         -- x が含まれる
      -- mf : [(CompFactor, Integer)]    -- このスロットも空

-- 定数 Factor を含むケース
def q : Poly Integer ['sqrt 2] [x] [] := ('sqrt 2) * x + 1
match q as poly integer with
  | $a :+ _ ->
    match a as term integer with
      | ($c, ($mc, $ms, $mf)) -> ...
      -- mc : ['sqrt 2 を含む可能性あり] / ms : [x の冪] / mf : []
...
`term` マッチャーで取り出した3スロットのモノミアルは、各スロット個別に `assocMultiset` で分解できる。各スロットは対応する Factor 分類のマッチャー（`constFactor`, `symbol`, `compFactor`）と組み合わせる。
...
match expr as poly integer with
  | $a :+ _ ->
    match a as term integer with
      | ($coeff, ($mc, $ms, $mf)) ->
        match (mc, ms, mf) as
          (assocMultiset constFactor, assocMultiset symbol, assocMultiset compFactor) with
          | (([], [(x, $n), (y, $m)], [])) -> ...  -- coeff * x^n * y^m

-- factor マッチャーでシンボルの構造を分解
match ('sin x) as compFactor with
  | apply1 $f $arg -> ...   -- f : sin 関数, arg : x

-- symbol マッチャーで名前とインデックスを取得
match x as symbol with
  | symbolName $name $indices -> ...   -- name : "x", indices : []
```
...
| 新しい数の導入   | ドメイン定義（SPAD） | Factor 3分類 + クォート演算子 `'`           |
...
3スロット設計（`ConstFactor` / `Symbol` / `CompFactor` の分離）を内部表現に反映する。
...
### Phase 2: 型システムへの統合（完了、ただし3スロット化で更新必要）
...
> - `TFactor` を `TConstFactor`, `TSymbol`, `TCompFactor`, `TFactor` の4種類に分割
...
CAS 型に対応する基本マッチャーを定義する。Factor の3分類に対応して `constFactor`, `symbol`, `compFactor`, `factor` の4種類を用意する。

```egison
def integer : Matcher Integer := something

-- 平のシンボル専用マッチャー
def symbol : Matcher Symbol :=
  matcher
    | symbolName $ $ as (something, list indexExpr) with
      | $tgt -> ...  -- extractSymbolName tgt で (name, indices) を抽出
    | #$val as () with
      | $tgt -> if tgt == val then [()] else []
    | $ as (something) with
      | $tgt -> [tgt]

-- 定数 Factor 専用マッチャー（自由シンボルなしの apply1/quote）
def constFactor : Matcher ConstFactor :=
  matcher
...
-- 合成 Factor 専用マッチャー（自由シンボルを含む apply1/quote）
def compFactor : Matcher CompFactor :=
  matcher
    | apply1 $ $ as (something, mathValue) with
      | $tgt -> ...
    | apply2 $ $ $ as (something, mathValue, mathValue) with
...
`integer` は `something` のエイリアス（型チェッカー完成後に制約を追加可能）。`constFactor` / `symbol` / `compFactor` / `factor` はそれぞれ `SymbolExpr` の構造を分解するマッチャーで、既存の `PDSymbolPat`, `PDApply1Pat`, `PDQuotePat` 等に対応するプリミティブ関数を使う。`constFactor` と `compFactor` は内部的に同じ `apply1`/`quote` を扱うが、ターゲットの自由シンボル有無で区別される。

- [ ] `integer`, `symbol`, `constFactor`, `compFactor`, `factor` マッチャーを `lib/math/expression.egi` に定義
...
def term {a} (m : Matcher a) : Matcher (Term a .. .. ..) :=
  matcher
    | ($, ($, $, $)) as (m,
                         assocMultiset constFactor,
                         assocMultiset symbol,
                         assocMultiset compFactor) with
      | $tgt -> [(termCoeff tgt,
...
- マッチャー引数の型からパターン変数の型を推論
  ...
  - `match a as term integer with | ($c, ($mc, $ms, $mf)) -> ...` で
    `c : Integer`, `mc : [(ConstFactor, Integer)]`, `ms : [(Symbol, Integer)]`, `mf : [(CompFactor, Integer)]` を推論

#### Step 5.6: テストと検証

- [ ] 基本テスト: `integer`, `symbol`, `constFactor`, `compFactor`, `factor`, `poly integer`, `frac integer`, `term integer`
- [ ] symbol/constFactor/compFactor テスト: 各分類専用マッチャーが対応する Factor のみにマッチすること
...
- 基本インスタンスの実装
  ...
  - `Embed ConstFactor Factor` — 型レベルのみの埋め込み
  - `Embed Symbol Factor` — 型レベルのみの埋め込み
  - `Embed CompFactor Factor` — 型レベルのみの埋め込み
  - `Embed ConstFactor (Poly Integer [..] [] [])` — cs スロットに入る
  - `Embed Symbol (Poly Integer [] [..] [])` — ss スロットに入る
  - `Embed CompFactor (Poly Integer [] [] [..])` — fs スロットに入る
...
各 `[..]` はフレッシュ型変数の糖衣構文であるため、`Embed Integer (Poly Integer [..] [..] [..])` は「任意の3スロット構成の `Poly Integer` への embed」として機能する。閉じた `Poly Integer [] [x] []` や `Poly Integer [] [x, y] []` に対して個別のインスタンスを定義する必要はない。

```
-- インスタンス宣言
instance Embed Integer (Poly Integer [..] [..] [..]) where ...
-- 脱糖後の内部表現（cs, ss, fs はフレッシュ型変数）
instance Embed Integer (Poly Integer [cs] [ss] [fs]) where ...

-- x + 1 の型チェック（x : Poly Integer [] [x] [], 1 : Integer）
-- 1. Embed Integer (Poly Integer [cs] [ss] [fs]) にマッチ
-- 2. cs = [], ss = [x], fs = [] にユニファイされる
-- 3. 結果: embed 1 : Poly Integer [] [x] []
```

同様に `Embed (Poly a [..] [..] [..]) (Poly b [..] [..] [..])` も、各 `[..]` が独立したフレッシュ型変数に脱糖されるため、任意の3スロット構成の組み合わせに対して機能する。
...
- 基本インスタンスの実装（`Math/CAS.hs` にランタイム検証関数）
  ...
  - `Coerce (Poly a [..] [..] [..]) ConstFactor` — 単一項で `monoConsts` のみが単一原子1乗かを検証
  - `Coerce (Poly a [..] [..] [..]) Symbol` — 単一項で `monoSymbols` のみが単一原子1乗かを検証
  - `Coerce (Poly a [..] [..] [..]) CompFactor` — 単一項で `monoComps` のみが単一原子1乗かを検証
...
- `Math/CAS.hs` に変換関数を追加
  ...
  embedIntToPoly (CASInteger n) =
    CASPoly [CASTerm (CASInteger n) (Monomial [] [] [])]

  -- Integer → Frac Integer
  embedIntToDiv :: CASValue -> CASValue
  embedIntToDiv (CASInteger n) = CASFrac (CASInteger n) (CASInteger 1)

  -- ConstFactor → Poly Integer [..] [] []  (cs スロットへ)
  embedConstFactorToPoly :: CASValue -> CASValue
  embedConstFactorToPoly (CASFactor sym) =
    CASPoly [CASTerm (CASInteger 1) (Monomial [(sym, 1)] [] [])]

  -- Symbol → Poly Integer [] [..] []  (ss スロットへ)
  embedSymbolToPoly :: CASValue -> CASValue
  embedSymbolToPoly (CASFactor sym) =
    CASPoly [CASTerm (CASInteger 1) (Monomial [] [(sym, 1)] [])]

  -- CompFactor → Poly Integer [] [] [..]  (fs スロットへ)
  embedCompFactorToPoly :: CASValue -> CASValue
  embedCompFactorToPoly (CASFactor sym) =
    CASPoly [CASTerm (CASInteger 1) (Monomial [] [] [(sym, 1)])]

  -- Poly a [cs₁] [ss₁] [fs₁] → Poly a [cs₂] [ss₂] [fs₂] (各スロット同方向): 内部表現は変わらない
...
**embed の原子集合拡大テスト** (`mini-test/61-embed-symbolset.egi`):
```egison
declare symbol x, y
def p : Poly Integer [] [x] [] := 1 + x
def q : Poly Integer [] [x, y] [] := p + y    -- ss スロットだけ拡大の自動 embed
```
...
**3スロット混在 embed のテスト** (`mini-test/63a-embed-three-slots.egi`):
```egison
declare symbol x
-- ConstFactor (cs スロット) と Symbol (ss スロット) の混在
def s : Poly Integer ['sqrt 2] [x] [] := ('sqrt 2) * x + 1
-- CompFactor (fs スロット) を含む
def t : Poly Integer [] [] [('sin x)] := 'sin x
```
...
`lib/math/analysis/derivative.egi` の `∂/∂` を `Differentiable` 型クラスのメソッドとして再実装する。3スロット化により、`fs = []` の制約を型レベルで表現できる。
...
- `Differentiable MathValue` インスタンス（一般ケース）の本体 `casDeriv` を実装し、`CASValue` のコンストラクタでパターンマッチ:
  ...
  - `CASFrac`: 商の微分法則 `(∂/∂ n s * d - n * ∂/∂ d s) / d^2`
  - `CASFactor (Apply1 ...)`: 連鎖律
- `Differentiable (Poly c [cs] [ss] [])` インスタンス（精密型）は `fs = []` 前提で直接実装し、`MathValue` 経由のオーバーヘッドを避ける
  - `poly m` + `term m` で各項を取り出し、`monoConsts` は無視（微分で消える）、`monoSymbols` だけを冪則で処理
  - `monoComps` のマッチャーは空であることを期待する（型レベルで保証されている）
- `Differentiable (Frac (Poly c [cs] [ss] []))` インスタンスは商の微分法則をそのまま表現
- テンソル対応は `tensorMap2` を使う既存のラッパーを維持
- 連鎖律（`Apply1` 等の合成関数の微分）は `MathValue` インスタンス内にのみ実装
- mini-test:
  - `∂/∂ ((x^2 + 3*x) : Poly Integer [] [x] []) x` → `2*x + 3 : Poly Integer [] [x] []`（ケースA選択の確認）
  - `∂/∂ ((('sqrt 2) * x^2 + 3*x) : Poly Integer ['sqrt 2] [x] []) x` → `2 * ('sqrt 2) * x + 3 : Poly Integer ['sqrt 2] [x] []`（cs スロットを含むケースA選択）
  - `∂/∂ ('log x) x` → `1/x : MathValue`（`CompFactor` → `MathValue` 経由のフォールバック確認）
  - `∂/∂ ('sin x : Poly Integer [] [] [('sin x)]) x` → `'cos x : MathValue`（`fs ≠ []` のフォールバック確認）
  - `∂/∂ p ('log x)` が型エラーになることを確認（第2引数が `Symbol` でない）
