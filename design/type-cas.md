...
-- 係数は CASValue（再帰的）: CASInteger, CASDiv, 別の CASPoly 等
...
#### CASFactor コンストラクタの要否

`Factor` 型の値を `CASValue` で表現する方法が未決定。

```haskell
-- 案A: CASFactor を入れる
data CASValue = CASInteger Integer | CASFactor SymbolExpr | CASPoly [CASTerm] | CASDiv CASValue CASValue

-- 案B: CASFactor を入れない（CASPoly で代用）
data CASValue = CASInteger Integer | CASPoly [CASTerm] | CASDiv CASValue CASValue
-- Factor は CASPoly [CASTerm (CASInteger 1) [(sym, 1)]] で表現
```

案A はクォート直後の値を自然に表現できる。案B はデータ型がシンプルになる。
...
-- 案A: ScalarData を CASValue に一括置換
data EgisonValue = ... | CASData CASValue | ...
-- Integer は CASData (CASInteger i) に

-- 案B: 段階的に並存
data EgisonValue = ... | ScalarData ScalarData | CASData CASValue | ...
```

案A は最終的な目標だが、既存コード（`EgisonData Integer` インスタンス、プリミティブ関数、パターンマッチ）への影響が大きい。案B は段階的な移行が可能。
