...
どちらも `Poly a [cs] [ss] [fs]` の `ss` スロット（第3引数）として同じ構文で使用される。
...
- `Differentiable MathValue` インスタンス（一般ケース）の本体 `casDeriv` を実装し、`CASValue` のコンストラクタでパターンマッチ:
  ...
  - `CASFrac`: 商の微分法則 `(∂/∂ n s * d - n * ∂/∂ d s) / d^2`
  - `CASFactor (Apply1 ...)`: 連鎖律
- `Differentiable (Poly c [cs] [ss] [])` インスタンス（精密型）は `casDeriv` を呼んで結果を coerce する（設計セクション1105行と同じ方針）。`casDeriv` の実装を一箇所に集中させ、保守性を優先する
  ```egison
  instance {Ring c} Differentiable (Poly c [cs] [ss] []) where
    ∂/∂ p s = coerce (casDeriv (unwrap p) s)
  ```
- `Differentiable (Frac (Poly c [cs] [ss] []))` インスタンスも同様に `casDeriv` を呼んで coerce する
