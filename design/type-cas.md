...
各マッチャーは最後の `$ as (m) with` 節で、自身のパターンにマッチしない場合に内部マッチャーへ**委譲**する。これは Egison の既存の `matcher` 構文で実現でき、新しい構文は不要：

```egison
def frac m :=
  matcher
    | $ / $ as (frac m, frac m) with
        | Div $p $q -> [(p, q)]
        | _ -> []
    | $ as (m) with                          -- m に委譲（分子として扱う）
        | $tgt -> [numerator tgt]
...
def term m :=
  matcher
    | term $ $ as (m, assocMultiset factor) with ...
    | mult $ $ as (m, multExpr) with ...
    | $ * $ as (m, multExpr) with ...
    | symbol $ $ as (...) with ...           -- factor ショートカット
    | apply1 $ $ as (...) with ...
    | ...
    | $ as (m) with                          -- m に委譲（係数として扱う）
        | $tgt -> [termCoeff tgt]
```
...
```egison
def mathExpr := frac (poly integer)
```

委譲チェーンにより、`mathExpr` で全層のパターンが利用可能：

```
match expr as mathExpr with
  | $n / $d          → frac が処理
  | poly $ts         → frac → poly に委譲（分子として扱う）
  | $a + $rest       → frac → poly に委譲
  | term $c $ms      → frac → poly → term に委譲
  | mult $c $rest    → frac → poly → term に委譲
  | symbol $name $i  → frac → poly → term に委譲（factor ショートカット）
  | apply1 $f $a     → frac → poly → term に委譲（factor ショートカット）
  | $x               → frac → poly → term → integer に委譲
```

委譲の各段階で値がどう変換されるか：

```
frac    : numerator              -- CASDiv を分子に変換（非 CASDiv はそのまま）
poly    : identity               -- CASPoly を1項として扱う
term    : termCoeff              -- CASTerm から係数を抽出（定数項なら値そのもの）
integer : （終端）
```

### `term` の factor ショートカット

`symbol`, `apply1`, `quote` 等のパターンは、`term m` マッチャーにショートカットとして組み込まれている。これにより `mathExpr` のトップレベルで直接使える。内部的には「係数1、モノミアル1要素、冪1」の場合にマッチする。

`factor` マッチャーは、`term m` の `assocMultiset factor` を通じたモノミアル分解でも使われる。
...
-- Poly パターン（全項を multiset で取得）
match expr as mathExpr with
  | poly $ts -> ...      -- ts : multiset of terms
...
-- Term パターン
match (3 * x) as mathExpr with
  | term $c _ -> c       -- 3

-- Symbol パターン（factor ショートカット）
match x as mathExpr with
  | symbol $name _ -> name    -- "x"

-- Apply1 パターン（factor ショートカット）
match ('sin x) as mathExpr with
  | apply1 _ $arg -> arg     -- x
...
match expr as poly (poly integer) with
  | $a + $rest ->
    match a as term (poly integer) with
      | term $c _ -> ...     -- c : Poly Integer [x]
```

### モノミアルと Factor の分解

`term` マッチャーで取り出したモノミアル（`[(Factor, Integer)]`）は `assocMultiset factor` でさらに分解できる：
...
- **`factor` ショートカット**: `symbol`, `apply1` 等のパターンが `term` に組み込まれており、`mathExpr` のトップレベルで直接使える
- **`$ as (m)` による委譲で合成可能**: `mathExpr := frac (poly integer)` のように、マッチャーの合成で上位層から下位層まで一貫してアクセスできる。新しい構文は不要
...
| 正規形の制御    | ドメインタワー      | 型注釈による `Poly`, `Div` の組み合わせ（マッチャーは `frac`, `poly`, `term`） |
...
### Phase 5: パラメトリックマッチャー（poly, frac, term）（完了）

旧モノリシック `mathExpr` マッチャーを、`list` や `multiset` と同様のパラメトリックなマッチャーに分解し、`mathExpr := frac (poly integer)` として再定義した。

#### 実装概要

`poly`, `frac`, `term` を純粋な Egison のマッチャー定義（`matcher` 式）として実装。各マッチャーの最後に `$ as (m) with | $tgt -> [f tgt]` 節を置くことで、内部マッチャーへの委譲を実現（新しい構文は不要）。
...
| マッチャー | パターン | 委譲先 |
|---|---|---|
| `frac m` | `$ / $` | `$ as (m) with numerator` |
| `poly m` | `poly $`, `$ + $`, `[]` | `$ as (term m)` |
| `term m` | `term $ $`, `mult $ $`, `$ * $`, `symbol $ $`, `apply1..4`, `quote $`, `func $ $` | `$ as (m) with termCoeff` |
| `factor` | `symbol $ $`, `apply1..4`, `quote $`, `func $ $` | — |
| `integer` | `something` のエイリアス | — |

`term m` に `symbol`, `apply1` 等の factor ショートカットを組み込んだことで、`mathExpr` のトップレベルから直接 `symbol $name _` 等のパターンが使える。
...
**追加したプリミティブ関数** (`Primitives/Arith.hs`):
- `casTerms` — CASValue を項のリストに分解
- `casFromTerms` — 項のリストから CASValue を構築
- `termCoeff` — 単一項から係数を抽出
- `termMonomial` — 単一項からモノミアルを抽出

**テスト**: `cabal test` 全21テスト通過、`mini-test/50-primitive-pattern.egi`, `mini-test/60-parametric-matcher.egi`, `mini-test/62-poly-div-term.egi` 全通過。
