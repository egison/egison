...
instance Differentiable MathValue where
  ∂/∂ v s = ...  -- CASInteger             → 0
                 ...
                 -- CASFactor (Apply1 f a)  → 常に連鎖律:
                 --   let f' = lookupDerivative f     -- DerivativeEnv から取得
                 --   in (Apply1 f' a) * ∂/∂ a s      -- (Apply1 f' a) は CASFactor に包む
...
**always-AF 規則の正しさ**: `Apply1 f a` を常に連鎖律で扱うと、`a` が定数（自由シンボルなし）の場合でも `∂/∂ a s` が再帰的に 0 を返すため、結果は `(Apply1 f' a) * 0 = 0` となり、数学的に正しい。従って `sqrt 2` や `log 3` のような「実質的に定数的な」ケースも正しく 0 に微分される。これによりランタイムの `classifyFactor` / `hasFreeSymbol` 解析は不要になる。
