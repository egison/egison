...
**`declare symbol` は runtime にも binding を作る**: `declare symbol K` は型レベルの登録に加え、評価環境にも `K → CASData (Symbol "K")` を pre-bind する (`Eval.hs:preBindDeclaredSymbols`)。pre-bind は **Phase 8 (`recursiveBindAll`) より前** に走る必要がある — そうしないと `def W := 1/(1-K*r^2)` のような closure が「K の binding が無い env」を capture してしまい、後で `W` を評価すると `IVarExpr "K"` が unbound 経由で大文字フォールバック (`InductiveData "K" []`) に落ちて `"Expected number, but found: K"` で失敗する。lowercase 名は `IVarExpr` の `symbolCASData` フォールバックで救われていたが、uppercase 名は constructor 扱いされる差で先に表面化した。pre-bind は両方を扱い、フォールバック経路への依存を排除する。
...
将来、以下のような構文でユーザがタワーを編集可能にする構想がある (構文・意味論は [type-cas-tower.md](./type-cas-tower.md) を正とする。以下はその要約):

```egison
-- ユーザ定義の中間型を宣言 (エイリアスは :=)
declare cas-type GaussianPoly := Poly (Poly Integer [i]) [..]

-- タワーへの位置を指定 (半順序の追加辺 — 関係のみを宣言する。
-- 埋め込み写像は処理系が casReshapeAs から導出し、ユーザは書かない。
-- D5 制限言語決定 (2026-07-04) により where embed 句は廃止)
declare cas-subtype Poly Integer [..] ⊂ GaussianPoly
declare cas-subtype GaussianPoly ⊂ Frac (Poly Integer [..])

-- 簡約規則: 従来どおりの global 宣言は「MathValue スコープの規則」として全型に波及
declare rule auto term i^2 = -1

-- 正規化 (canonicalize) のフック: MathValue からの retraction
...
**規則の帰属の原則 (2026-07-04 決定)**: `declare rule` は従来どおり **`MathValue` 大域のみ**で、型スコープ規則 (`in T`) は導入しない。等式理論の置き場は 3 分類で完結する — (1) **タワー型**は単一の大域理論を共有 (大域 `declare rule auto`、現状のまま)、(2) **シンボルが関係式を担う商** (`i^2 = -1`、`ε^5 = 0`、1 の冪根) はシンボルの大域一意性がスコープを代替するので大域規則で健全、(3) **係数領域の商** (ℤ/nℤ 等) はタワー外の商機構 ([type-cas-quotient.md](./type-cas-quotient.md) — `declare cas-quotient Q := T by reduce`、順序不参加、per-op reduce、型 dispatch される Eq) の `reduce` に封じ込める。タワー型に「追加の等式」を課したくなったら、それは商の偽装なので (3) を使う。`canonicalize` は**構造選択** (どの入れ子・どのレベルの canonical form か) のみを担い、規則環境には触れない (規則機構は現状実装から変更なし)。
