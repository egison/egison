...
`declare symbol`, `declare mathfunc`, `declare apply`, `declare rule`, `declare ideal`, `declare derivative`, `declare cas-type`, `declare cas-subtype`, `declare cas-quotient` の **`declare` 系宣言**は、ロード単位ごとのプリパス (`EnvBuilder.buildEnvironments`、cas-quotient のみ `Eval.expandCasQuotientDecls`) で収集・検証され、以降の型推論・評価から参照される。

**実装形態は 2 種の組み合わせ** (単一の `DeclareEnv` レジストリは採用しなかった):

1. **desugar が生成する通常の定義**: 宣言の「本体」は普通の `def` に脱糖される。
   - `declare rule auto` → `def autoRule.<n> := ...` + `mathNormalize` の再定義 (`iterateRulesCAS` 呼び出し)
   - `declare rule <name>` → `def rule.<name> := ...` (`simplify e using name` は `rule.<name> e` に脱糖)
   - `declare ideal [gens]` → `def idealRules.<n> := idealTermRules [π] [gens']` + `def autoRule.<n> := \v -> applyIdealRules idealRules.<n> v` + `mathNormalize` 再定義 (2026-07-06、G3)。生成元はグレブナー基底に完備化され term 級書き換えペア (LT, LT −' g) になる。GB 計算は初回発火時に 1 回 (遅延 define・自由理論エンジンなので自己再入なし)。π = `declare symbol` の宣言順 (EvalState.declaredSymbolOrder) ++ 生成元 AST 内出現順の複合原子。[cas-simplification.md](./cas-simplification.md) G3 実施結果参照
   - `declare mathfunc f` → シンボリックラッパー `def f := \x -> 'f x`
   - `declare apply f x := body` → ラッパーを上書きする `def f := \x -> body`
   - `declare derivative f = g` → `def deriv.<f> := g` + `chainPartialDiff` の再定義
   - `declare cas-quotient Q := T by reduce` → `projQ`/`reprQ`/Ring インスタンス一式のテンプレート展開
2. **`EvalState` の型付きフィールド**: 検査・最適化用のメタデータのみ個別フィールドで持つ。
   `autoRuleVarNames` / `autoRuleTriggers` (trigger filter 用の Set キャッシュ)、`ruleNames`、
   `reductionRulesCount` / `derivativeRulesCount`、`casTypeAliasEnv`、`casSubtypeEdges` など。

**`class` / `instance` 宣言は別管理**: 型クラス機構は `EvalState` の独立フィールド (`classEnv` / `instanceEnv`) で管理され、構文もパーサ経路も別。

#### 重要な性質

**宣言の順序制約はない**: プリパスで全宣言を先に集めるので、`declare mathfunc sqrt` より前の位置に `def f : Poly Integer [sqrt 2] := ...` と書いても有効。

**相互参照が可能**: `declare apply sqrt x := ... sqrt ... ` のような再帰参照や、`declare rule` の RHS が他の `declare mathfunc` 関数を呼ぶことも許される。

**正規化は宣言に依存する**: 自動規則は `mathNormalize` (算術演算のたびに呼ばれる) を再定義するかたちで効く。同じ式でもロード済みの `declare rule auto` が異なれば結果が変わる。

**自動規則はロードバッチ単位で効く (2026-07-06 判明)**: 再帰束縛のため、同一バッチ内では**ファイル末尾の `declare rule auto` / `declare ideal` が先頭の式の評価にも効く** (宣言位置は同一バッチ内で意味を持たない)。逆に、lib の演算子クロージャは自バッチの `mathNormalize` を捕捉するので、**別バッチで宣言した規則は既ロードの演算子から見えない** — CLI がテストファイルを初期ロードと同一バッチに含めるのはこのため。test/Test.hs も同方式に修正済み (旧: lib とテストファイルを別バッチで評価 → テストファイル内の規則宣言が発火しないバグ)。

**ロード時のエラー検出**: 宣言の整合性 (例: `declare apply f` の `f` が `declare mathfunc` 済みか、cas-type エイリアスの重複・循環、cas-subtype の D1 半束検査、入れ子 Poly タワー内の `[..]` 高々 1 つ) はプリパスで検証する。

**`declare symbol` は runtime にも binding を作る**: `declare symbol K` は型レベルの登録に加え、評価環境にも `K → CASData (Symbol "K")` を pre-bind する (`Eval.hs:preBindDeclaredSymbols`)。pre-bind は **`recursiveBindAll` より前** に走る必要がある — そうしないと `def W := 1/(1-K*r^2)` のような closure が「K の binding が無い env」を capture してしまい、後で `W` を評価すると `IVarExpr "K"` が unbound 経由で大文字フォールバック (`InductiveData "K" []`) に落ちて `"Expected number, but found: K"` で失敗する。pre-bind は lowercase / uppercase の両方を扱い、フォールバック経路への依存を排除する。

#### `declare-key` 構想 (未着手)

`declare derivative` はユーザーランドの型クラス `Differentiable` の補助データという性質を持つ。類似機構 (`Integrable` 等) が増えた場合に備え、`declare-key derivative` で宣言キーを予約してライブラリ側で処理を書ける一般化構想がある (「実装状況 > Phase 9」参照)。現状は built-in 構文のままで実害はない。
...
##### 入れ子 Poly タワー内の `[..]` は高々1つ (2026-07-06 決定・実装済)

入れ子 Poly の**タワー**（係数位置の Poly/Term の連なり。Frac を透過して降りる）の中では、
`[..]` は**高々1箇所**しか現れてはならない。開いたスロットが1つなら各原子の行き先が一意に決まる
（閉じた集合に載っている原子はその段へ、どこにも載っていない原子は唯一の開いた段へ）が、
2つあると余り原子の行き先が曖昧になり reshape が定義できないため。
タワーの外（タプル成分・関数引数など。将来 Frac の分子と分母が別型を取るならそれぞれ）は
別タワーなので、各タワーに1つずつ `[..]` を持ってよい。

- 検査は `Types.hasAmbiguousOpenTower`。フックは3箇所:
  式注釈（`Infer.hs` の `IReshape`、`UnsupportedFeature` 型エラー）、
  `def` シグネチャ収集（`EnvBuilder.hs` の `DefineWithType`）、
  `declare cas-type`（エイリアス展開**後**に検査。alias 合成で2つになるケースを捕捉）と
  `declare cas-subtype`（両辺）。
- この制限の下で reshape（`CAS.hs` の `reshapeAsPoly`）の原子振り分けは:
  - 内側タワーが全閉（外側は閉でも `[..]` でも可）: 内側タワーの閉集合の**深い合併**に
    載っている原子を係数側へ（3段以上のタワーも正しく振り分け。旧実装は直下の段しか見ず、
    `Poly (Poly Integer [i]) [..]` 形しか動かなかった）。
  - 内側タワーに開スロットがあり、この段が閉（例 `Poly (Poly Integer [..]) [i]`）:
    **補集合分割** — この段の閉集合の原子だけ残し、それ以外を係数側へ。
  - 振り分け情報なし（係数が基底型、または静的検査が弾くはずの両開）: 分離せず係数を再帰
    （basic widening、従来どおり）。
- テスト: `mini-test/132-open-inner-poly-reshape.egi`
  （閉閉/外開/内開/3段全閉/3段最内開の reshape と値保存、拒否4経路はコメントに記載）。
...
分数の正規化では、まず `casTermsGcd` (`Math/CAS.hs`) による**モノミアルGCD**簡約が走る：
...
対象は「分子・分母が**同一の単一シンボル**の単変数多項式で、冪がすべて正、
係数が整数または整数分数」の場合のみ (次数上限 200)。ℚ 上の Euclid 互除法で
GCD を計算し、`(x^2 - 1)/(x - 1) → x + 1` のように約分する。約分後の分子・分母は
**共通の**倍率で「整数係数・joint content 1・分母先頭係数正」に正規化される
(分子と分母を常に同じ定数倍するので値は厳密に保存)。分母が定数や単項式に潰れた
場合は `casNormalizeFrac` に再入して既存の吸収規則 (係数 Frac 化・ローラン化) が
適用される。スコープ外の入力 (多変数、ローラン、シンボリック係数) は従来どおり
未約分の `CASFrac` に落ちる。多変数化 (content/subresultant PRS) は第 2 段として
保留。検収: mini-test/131。

#### グレブナー基底ライブラリ (明示適用)

自動の GCD 約分とは別に、**イデアルを法とする正規形**は `lib/math/algebra/groebner.egi`
のライブラリ関数で明示的に計算できる (2026-07-06 追加、Egison 自身で実装):

```egison
groebnerBasis     : [MathValue] -> [MathValue]   -- 被約 GB (Buchberger)
...
単項式順序は grevlex 固定。非多項式・ローランは fail-open (入力をそのまま返す)。
`polyNF gb f = 0` がイデアル所属判定。演算経路には入れない (発火点は増やさない) —
宣言時規則生成 (`declare ideal`、**G3 実装済** — 上の declare 系一覧参照) の値レベルエンジン。設計・実施結果は
[cas-simplification.md](./cas-simplification.md) §3.3/G2、実行例は
`sample/math/algebra/groebner-basis.egi`、検収は mini-test/134。
...
実物 (`lib/math/algebra/root.egi` / `lib/math/normalize.egi`) は以下のとおり:
...
--    実装は n 乗根 rt に委譲する: rt が完全平方因子の抽出
--    (sqrt 4 → 2, sqrt 8 → 2 * sqrt 2) と記号引数の素通し
--    (sqrt (x+1) → 'sqrt (x+1)) を担う
declare apply sqrt x :=
  let m := numerator x
      n := denominator x
   in rt 2 (m *' n) /' n
...
上記の組み込みタワーは **CAS 標準型のみ** を対象にしているが、ユーザは以下の宣言でタワーを拡張できる (設計経緯と決定 D1–D5 は [type-cas-tower.md](./type-cas-tower.md)、実装フェーズの記録は [type-cas-tower-implementation.md](./type-cas-tower-implementation.md)):

```egison
-- ユーザ定義の正規形に透明なエイリアスを与える (Phase α)
declare cas-type GaussianPoly := Poly (Poly Integer [i]) [x]

-- タワーへ辺を追加する (Phase β)。関係のみを宣言し、埋め込み写像は
-- 処理系が casReshapeAs から導出する (D5: where embed 句は無い)。
-- 宣言のたびに join の一意性 (D1 半束性) が検査され、壊れる宣言は
-- 完備化辺の提案つきでエラーになる。
declare cas-subtype Poly Integer [i, x] <: GaussianPoly

-- 簡約規則は従来どおり大域 (規則の帰属の原則、下記)
declare rule auto term i^2 = -1
```

これにより `join` はユーザ追加型を含めた半順序の上で計算され、暗黙の昇格・型注釈の reshape が中間型 (Gaussian 整数係数の多項式など) に着地できる。旧構想にあった `class CASCanonical` / `canonicalize` フックは**廃案** — 演算結果は常に既定の平坦形で返り (Phase γ′ の吸収律 `casReshapeAs C . casReshapeAs B = casReshapeAs C`)、入れ子形は注釈を書いた場所にだけ現れるので、型ごとの正規化フックは不要になった。

**規則の帰属の原則 (2026-07-04 決定)**: `declare rule` は従来どおり **`MathValue` 大域のみ**で、型スコープ規則 (`in T`) は導入しない。等式理論の置き場は 3 分類で完結する — (1) **タワー型**は単一の大域理論を共有 (大域 `declare rule auto`、現状のまま)、(2) **シンボルが関係式を担う商** (`i^2 = -1`、`ε^5 = 0`、1 の冪根) はシンボルの大域一意性がスコープを代替するので大域規則で健全、(3) **係数領域の商** (ℤ/nℤ 等) はタワー外の商機構 ([type-cas-quotient.md](./type-cas-quotient.md) — `declare cas-quotient Q := T by reduce`、順序不参加、per-op reduce、型 dispatch される Eq、**実装済**) の `reduce` に封じ込める。タワー型に「追加の等式」を課したくなったら、それは商の偽装なので (3) を使う。
...
**実装状況 (2026-07-05 配線済み)**: 実装は「単一化ファースト + 適用位置 join」の二段構え。
単一化そのものが互換ケース (原子集合の包含方向、係数の昇格等) を吸収し、CAS 型同士の単一化が
失敗したとき (実質: 非可換な閉原子集合や、宣言辺でのみ関係づく正規形同士) に限り、apply 推論が
`joinTypesWith` (骨格 + `declare cas-subtype` 辺の宣言順序) で一意 join を計算し、join 以下の
CAS 引数へ `reshape` を挿入して単一化を 1 回だけ再試行する (`Infer.inferIApplicationWithContext`)。
**join を unifier には入れない** — join は coercion (D5: `casReshapeAs` のみ) であり、単一化の
対称性と主型性を保つ。商型 (nominal、非 CAS) には発火しないため明示横断の意味論は不変。
検収: `design/cas-tower-usecases/06` (自然形 `a + b`)、mini-test/130。
...
instance {Ring a, atoms} AddMonoid (Poly a atoms) where
  (+) p q := mathNormalize (casPlus (unwrap p) (unwrap q))
...
instance {Ring a} MulMonoid (Poly a [..]) where
  (*) p q := mathNormalize (casMult (unwrap p) (unwrap q))
...
-- 簡約規則の適用は最終結果に対して mathNormalize で行う
-- (lib/core/base.egi の実装は plusForMathValue / multForMathValue 経由で
--  mathNormalize を呼ぶ; mathNormalize は declare rule auto ごとに再定義される)
instance Ring MathValue where
  (+) a b := mathNormalize (casPlus a b)
  (*) a b := mathNormalize (casMult a b)
  zero := CASInteger 0
  one := CASInteger 1
  neg := casNeg
...
**無限再帰の懸念について**: 「`#x` 照合 → `Eq MathValue` → `mathNormalize` → 同じ規則が再びマッチして `#x` 照合 → ...」のような再帰が起きないかという懸念がある。実用上は問題にならない:
...
実装上は念のため、`#x` 照合中の `mathNormalize` 呼び出しでは規則適用の再帰深度を制限する等のガードを入れてもよいが、必須ではない。
...
規則は専用のレジストリではなく、**desugar が生成する普通の定義**として存在する
(`Desugar.hs` の `DeclareRule` ケース):

- `declare rule <name> <level> <lhs> = <rhs>` → `def rule.<name> := <一段書き換えラムダ>`
- `declare rule auto <level> <lhs> = <rhs>` → `def autoRule.<n> := <同>` に加えて
  `def mathNormalize := \v -> iterateRulesCAS [autoRule.0, ..., autoRule.n] (mathNormalizeBuiltin v)`
  と **`mathNormalize` を再定義**する (自動規則は算術演算のたびに発火する)

一段書き換えラムダの作り方は LHS の形で分岐する:

- **リテラル LHS** (パターン変数なし。例 `i^2 = -1`): term 級は `applyTermRule lhs rhs v`
  (モノミアル包含マッチ — `3 * i^2 * x` の中の `i^2` にも当たる)、poly/frac 級は等値比較。
- **パターン LHS** (`$x`/`#x` を含む): `match v as mathValue with | <lhs> -> <rhs> | _ -> v` の
  一点書き換えを、級に応じた走査プリミティブ `mapTermAll` / `mapPolyAll` / `mapFracAll` で包む。
  これらは Apply1–4 / Quote / Function の引数へも再帰するので、規則は任意の部分式で発火し、
  各ノードで不動点まで反復される。表層の `sin $x` は `apply1 #sin $x` に翻訳される。

固定点ループは Haskell プリミティブ `iterateRulesCAS` が回す。RHS ではユーザー定義関数を
含む任意の Egison 式が使える。

### 規則適用のトリガーフィルタ

**正しさには影響しない最適化**。宣言時に LHS から**トリガーシンボル集合**
(リテラル参照のシンボル `i` や、パターン参照の関数名 `sqrt` など) を抽出し、
`EvalState.autoRuleTriggers :: [Set String]` に Set 化して保存する (毎演算での再構築を避ける)。
2026-07-07 に**適用パターンは頭の関数名のみ**へ精密化した (引数内のシンボルは
トリガーに含めない): `log (exp $n)` のトリガーは {log, exp} ではなく {log}。
マッチする項は必ず頭の因子を含むので健全で、トリガー集合が小さいほど
スキップが増える (exp だけの値で log 規則が試行されなくなる)。
`iterateRulesCAS` は反復のたびに値に現れるシンボルを 1 回走査し、
トリガー集合と交わらない規則のマッチングをスキップする。

- フィルタは**値ベース** (値に実際に現れるシンボルで判定)。設計初期に検討した
  静的型ベースのフィルタ (`atoms(T)` との交差判定) は、値が自分のシンボルを
  持っているのでこの形に吸収された。閉じた原子集合の型に対しては値ベース判定が
  そのまま完全になる (トリガーが値に無ければ規則は発火し得ない)。
...
各フェーズの現状の要約と、そこで得られた設計知見の記録。未着手の項目は「既知の制限と未解決課題」にまとめてある。
...
**残されたサブ課題**: `Apply1〜Apply4` の `ApplyN` 一般化のみ（「既知の制限 > 意図的に実装しない項目」参照）。
...
- FunctionDataの導関数を下流IRへ構造的に渡す用途には
  ...
  Sub/Sup/Userを区別できない。`symbolIndices`はCAS symbolを直接検査し、
  `SubIndex` / `SupIndex` / `DiagIndex` / `UserIndex`を保持して返す。これにより
  pretty-print文字列を解析せず、tensor component indexと位置正準形の
  FunctionData derivative indexを分離できる。
...
**現状サマリー**
- `Math/Rewrite.hs`: `casRewriteDd` 1 関数 + `casHasFunctionData` のみ（性能上の最適化として意図的に Haskell 残置。詳細は `lib/math/normalize.egi` 末尾コメント参照）。`Math/Normalize.hs` は importer が無くなったため削除済 (2026-07-06)
- 自動規則は `lib/math/normalize.egi` に `declare rule auto` で定義
- `declare apply` 化済の関数: `sqrt`, `rt`, `rtu`, `sin`, `cos`, `exp`, `log`, `abs`
- 性能は「サンプル想定実行時間」の表を参照
...
**2b. 規則抑制クオート `'( )` (2026-07-06、G3 と同時導入)**

上の `unNormalizeOps` 変換 (規則 RHS が暗黙に受ける「自由理論の構造演算で組む」扱い) を、ユーザーが**任意の式で明示的に**使える構文にしたもの。`'(式)` は `declare rule` の書き換えを発火させずに式を構築する (構造正規化 — 展開・同類項マージ — はする):

```egison
'((sin θ)^2 + (cos θ)^2 - 1)   -- 'sin θ^2 + 'cos θ^2 - 1  (素の式は 0 に潰れる)
'(w^2 + w + 1)                 -- w^2 + w + 1
'(2 * x + x)                   -- 3 * x   (構造正規化は生きる)
```

実装は desugar 1 ケース: `QuoteSymbolExpr` の内側が desugar 後 `IVarExpr` なら従来の関数クオート (`'f`・`'(^)`)、それ以外は `unNormalizeOps` を適用。従来の `'(非変数)` は評価素通しの no-op だったため後方互換の実害なし。クオートの使い分け: `` ` `` = 構造を凍結 (原子化)、`'f` = 適用を凍結、`'( )` = 理論 (規則) を凍結。イデアル生成元 (`polyNF ['((sin θ)^2 + (cos θ)^2 - 1)] e`) と規則 RHS の可読形が主用途。検収: mini-test/135。
...
| ~~関数シンボルの CAS 型統合~~ **仕様確定 (2026-07-07)** | 導関数索引を**位置正準形**に統一 ([function-symbol.md](./function-symbol.md) 冒頭の確定仕様参照): userRefs が構築時に値→位置解決 (非引数/曖昧/範囲外はエラー)・多重索引ソート (Schwarz)・冪下の括弧表示。**値レベルは完全統合済み** (GB・declare rule・トリガーが FunctionData を扱うと実測)。型は `MathValue`、型レベル原子集合出現禁止は維持 (開放は extendAtoms 経路で可能) | 完了 (残 = 型レベル原子集合の開放、必要時) |
| ~~tensor index 評価バグ~~ **解消済 (2026-07-07)** | (1) `pmIndex` の `Sub Nothing`/`Sup Nothing` ケース欠如 → 修正済 (Data/Utils.hs)。(2) `tref` の過剰 index メッセージ改善済 (Tensor.hs)。(3) 行列×ベクトルは `MV.*` として lib 化 (`lib/math/algebra/matrix.egi`; `M.*` は右被演算子を 2 回添字するので rank 2 が必要 — ベクトルは共有添字 1 本で縮約)。`tribonacci.egi` は lib の `MV.*` で A¹⁰⁰·B まで全 assert 成立 | 完了 |
...
| ファイル | 想定 |
|---|---:|
| `canonical-form-absorption.egi` | 1s (2026-07 追加: 注釈による正規形選択と吸収律の公開検証。論文 §3 の実行物) |
...
| `hodge-E3.egi` | 3s |
| `kahler-geometry-of-CP1.egi` | 1s (2026-07 追加: Wirtinger 計算での FS 計量・Kähler-Einstein・固有関数。拡張タワーのデモ) |
...
| `elliptic-curve-over-F7.egi` | 1s (2026-07 追加: 𝔽₇ 上の楕円曲線の点数え+群法則。商機構のデモ) |
...
`scripts/sample-timings.txt` に `<file> <expected_seconds>` 形式で列挙し、
`scripts/run-sample-suite.sh` が `gtimeout $((expected * 3 / 2))` で実行
(最低 5s, FAIL カテゴリは 300s 固定)。

```sh
scripts/run-sample-suite.sh                # 全 sample
scripts/run-sample-suite.sh sample/foo.egi # 単独 (timing 表に無い場合は 30s デフォルト)
```

合計実行時間 (per-sample timeout): 約 **30 分** (想定値 × 1.5 + FAIL の 300s × 5 件)。
最終結果: pass=40, fail=6 (87% PASS、2026-05 時点)。
