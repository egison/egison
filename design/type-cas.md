...
-- どのスロットに入るかはユーザーの型注釈（Poly の cs/ss/fs 指定）で決まる
data Monomial = Monomial
  { monoConsts  :: [(SymbolExpr, Integer)]  -- ConstantFactor の冪の積（cs スロット由来）
  , monoSymbols :: [(SymbolExpr, Integer)]  -- Symbol の冪の積（ss スロット由来）
  , monoComps   :: [(SymbolExpr, Integer)]  -- AppliedFactor の冪の積（fs スロット由来）
  }
-- 冪指数は負も可（ローラン多項式のため）
```
...
- `freeSymbols` のランタイム解析は不要（ユーザー注釈を信頼）
- `Poly` に embed されると3スロットのどこに入るかで分類が実体化する（`ConstantFactor` は `monoConsts`、`Symbol` は `monoSymbols`、`AppliedFactor` は `monoComps`）
- `Differentiable` 等のインスタンスディスパッチは**型**で行う（値の構造は見ない）
...
- [ ] Factor の3分類は **ユーザーの型注釈ベース**（`declare symbol` 経由なら `Symbol`、`declare mathfunc` 経由なら呼び出し側の注釈で決まる）。`freeSymbols` による自動判定は採用しない
- [ ] `casNormalizePoly`, `casTermsGcd`, `monoGcd` 等を3スロットに対応させる
- [ ] `casPlus`, `casMult` のモノミアル比較・結合ロジックを3スロット独立に
- [ ] 既存の単一リスト前提のコードを全て修正
- [ ] どのスロットに入るかは `Poly` への埋め込み時に型情報から決まる（`Embed ConstantFactor (Poly Integer [..] [] [])` は cs に、`Embed Symbol (Poly Integer [] [..] [])` は ss に、等）
...
`constantFactor` / `appliedFactor` / `symbol` は、3スロットモノミアルの対応スロット（`monoConsts` / `monoComps` / `monoSymbols`）の要素にマッチする目的で使う。`term` マッチャーの3スロット分解経由で使うのが主で、型情報により**どのスロットから取り出すか**が静的に決まる。既存の `PDSymbolPat`, `PDApply1Pat`, `PDQuotePat` に対応するプリミティブ関数を使う。
