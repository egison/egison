...
-- タワーへの位置を指定 (半順序の追加辺、canonical embedding 付き)
declare cas-subtype Poly Integer [..] ⊂ GaussianPoly where
  embed v := primCASReshape v <GaussianPoly>
declare cas-subtype GaussianPoly ⊂ Frac (Poly Integer [..]) where
...
**規則の帰属の原則 (2026-07-04 統一)**: 等式理論 (`declare rule`) は型にスコープする。既存の global `declare rule auto` は「`MathValue` スコープの規則」と読み替え、部分順序の下位すべてに継承されるので後方互換。`ℤ/nℤ` のように他の型へ漏れてはならない理論のみ型スコープ規則 (構文候補: `declare rule auto term ... = ... in T`) で書く。`canonicalize` は**構造選択** (どの入れ子・どのレベルの canonical form か) を担い、型スコープ規則の固定点適用を含む。型スコープ規則の発火点 (coerce/reshape 境界のみか、静的型による演算ごとの dispatch か) は未決 ([type-cas-tower.md §8 未決事項](./type-cas-tower.md) D2)。
