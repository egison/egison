...
`sin`, `cos` 等の数学関数を「CAS の数学関数」として明示的に宣言する。
...
返り値型は `MathValue` である。`declare apply` が定義されていない場合のデフォルトの挙動は、`CASFactor (Apply1 ...)` を生成する（引数に自由シンボルがあれば `AppliedFactor`、なければ `ConstantFactor`）。`declare apply` が定義されている場合は、その評価結果が返る（`Integer` になることも `Poly` になることもある）。

呼び出し側で具体的な型が必要な場合は、式レベル型注釈（後述）で `coerce` を挿入する:

```egison
sqrt 2                              -- : MathValue
(sqrt 2 : ConstantFactor)           -- : ConstantFactor（coerce 挿入、成功）
(sqrt 4 : Integer)                  -- : Integer（coerce 挿入、sqrt 4 = 2 なので成功）
(sqrt 8 : Poly Integer [sqrt 2] [] [])  -- : Poly Integer [sqrt 2] [] []（coerce 挿入、成功）
```
...
(sqrt 2 : ConstantFactor)                  -- sqrt 2 : ConstantFactor
