...
`declare symbol`, `declare mathfunc`, `declare apply`, `declare rule`, `declare derivative`, `declare cas-type`, `declare cas-subtype`, `declare cas-quotient` の **`declare` 系宣言**は、ロード単位ごとのプリパス (`EnvBuilder.buildEnvironments`、cas-quotient のみ `Eval.expandCasQuotientDecls`) で収集・検証され、以降の型推論・評価から参照される。
...
宣言時規則生成 (`declare ideal`、G3) の値レベルエンジン。設計・実施結果は
