...
**`declare derivative` の登録 (Phase 6.3 part 3)**:
...
- [ ] 実際の `Differentiable Factor` ディスパッチで `ApplyN ref args` の `lookupDerivative` への接続は依然 pending
...
- [x] **Step 7.4 → 7.5 (partial)**: 集めた declare rule を `EnvBuildResult.ebrReductionRules` に格納、件数を `EvalState.reductionRulesCount` に伝搬。`numReductionRules ()` プリミティブで件数取得可能 (`mini-test/83-rule-registration.egi`)。実際の rule 適用 (LHS パターンを CASValue → CASValue 関数にコンパイルし、casNormalize 内で繰り返し適用) は依然 pending。
