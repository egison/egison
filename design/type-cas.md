...
`'` は関数適用を停止して `ConstantFactor` または `AppliedFactor` の値を生成する組み込み演算子。生成される値の型は引数の自由シンボル有無で決まる。

```egison
'sqrt 2         : ConstantFactor     -- √2 を表す原子（引数 2 に自由シンボルなし）
'sin x          : AppliedFactor      -- sin(x) を表す原子（引数 x に自由シンボルあり）
'sin ('sqrt 2)  : ConstantFactor     -- sin(√2) を表す原子（引数 'sqrt 2 にも自由シンボルなし）
```
...
sqrt n =
  ...
  else embed ('sqrt n)                            -- ConstantFactor → Poly Integer [..] [..] [..] の embed
```

- `declare rule auto ('sqrt $x)^2 = x` はトップレベル宣言で、任意の `x` に対して `('sqrt x)^2` を `x` に簡約する
- 完全平方数のときは整数を開いた `Poly` に embed して返す
- そうでないとき、`'sqrt n` を生成し `Poly` に embed して返す。`n` は整数なので結果は `ConstantFactor`
...
match expr as poly integer with
  | $a :+ _ ->
    match a as term integer with
      | ($c, ($mc, $ms, $mf)) -> ...
      -- c  : Integer
      -- mc : [(ConstantFactor, Integer)]   -- このスロットは空（[] [x] [] なので）
      -- ms : [(Symbol, Integer)]         -- x が含まれる
      -- mf : [(AppliedFactor, Integer)]    -- このスロットも空
...
`term` マッチャーで取り出した3スロットのモノミアルは、各スロット個別に `assocMultiset` で分解できる。各スロットは対応する Factor 分類のマッチャー（`constantFactor`, `symbol`, `appliedFactor`）と組み合わせる。
...
def constantFactor : Matcher ConstantFactor :=
  matcher
...
def appliedFactor : Matcher AppliedFactor :=
  matcher
    | apply1 $ $ as (something, mathValue) with
      | $tgt -> ...
    | apply2 $ $ $ as (something, mathValue, mathValue) with
...
`integer` は `something` のエイリアス（型チェッカー完成後に制約を追加可能）。`constantFactor` / `symbol` / `appliedFactor` / `factor` はそれぞれ `SymbolExpr` の構造を分解するマッチャーで、既存の `PDSymbolPat`, `PDApply1Pat`, `PDQuotePat` 等に対応するプリミティブ関数を使う。`constantFactor` と `appliedFactor` は内部的に同じ `apply1`/`quote` を扱うが、ターゲットの自由シンボル有無で区別される。
...
- `Math/CAS.hs` に変換関数を追加
  ...
  -- ConstantFactor → Poly Integer [..] [] []  (cs スロットへ)
  embedConstantFactorToPoly :: CASValue -> CASValue
  embedConstantFactorToPoly (CASFactor sym) =
  ...
  -- AppliedFactor → Poly Integer [] [] [..]  (fs スロットへ)
  embedAppliedFactorToPoly :: CASValue -> CASValue
  embedAppliedFactorToPoly (CASFactor sym) =
    CASPoly [CASTerm (CASInteger 1) (Monomial [] [] [(sym, 1)])]
...
```egison
declare symbol x
-- ConstantFactor (cs スロット) と Symbol (ss スロット) の混在
def s : Poly Integer ['sqrt 2] [x] [] := ('sqrt 2) * x + 1
-- AppliedFactor (fs スロット) を含む
...
- mini-test:
  ...
  - `∂/∂ ('log x) x` → `1/x : MathValue`（`AppliedFactor` → `MathValue` 経由のフォールバック確認）
