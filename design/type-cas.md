...
理由: 型注釈の意味論的明確性(同じ原子が複数の分類に属すると意味が曖昧になる)と、将来的にスロット別操作を追加する際の一貫性のため。ランタイム意味論(coerce や term マッチャー)は和集合 `cs ∪ ss ∪ fs` として扱うので disjoint は必須ではないが、型の意味論として要求する。
...
Type ADT に `TPoly Type SymbolSet SymbolSet SymbolSet` を追加し、`SymbolSet` は `Closed [SymbolExpr]` / `Open` の2構成子。3つのスロットがそれぞれ独立に閉じる/開くことができる。

**文法設計**: `Poly T [S1] [S2] [S3]` は **3 スロット専用構文**として 3 つの角括弧グループを必須とする。これによりコレクション型 `[T]` との曖昧性を解消する:

```
<poly-type> ::= 'Poly' <type-atom> <slot-set> <slot-set> <slot-set>
<slot-set>  ::= '[' '..' ']'           -- open slot
...
`Poly T [x, y]` のような 1 スロット記法は**糖衣構文の範囲**(`ConstantPoly`, `SymbolPoly` 等)のみで許可し、生の `Poly` 型は常に 3 スロットを要求する。これによりパースの局所性が保たれる。
...
- `coerce : MathValue → Poly a [cs] [ss] [fs]`（所属検証、ただしスロット別ではなく `cs ∪ ss ∪ fs` の和集合に対する検証）
- `Differentiable MathValue` のディスパッチ（always-AF 規則で分類なしに動作）

`term m` マッチャーはランタイム Monomial が flat であることを反映し、**1 スロット(flat)で分解**する(`($c, $m)` で `$m : [(Factor, Integer)]`)。3 スロットへの振り分けはマッチャー側では行わない。ユーザーが分類したい場合は、`$m` の各要素に対して `symbol`/`constantFactor`/`appliedFactor` マッチャーを自分で適用する。

`casNormalize` の内部ループでは分類判定を**行わない**（正規化順序に分類が依存する問題を回避）。詳細は「境界での名前集合照合」セクションを参照。
...
1. `coerce : MathValue → Poly a [cs] [ss] [fs]` の所属検証(スロット別ではなく和集合 `cs ∪ ss ∪ fs` に対する検証)
2. `Differentiable MathValue` のディスパッチ(always-AF 規則)

どちらもワンショット操作であり、正規化ループの内側で走らないので、「`sin(x-x)` の正規化で分類が変わる」問題（L468 参照）は起きない。

`term m` マッチャーは 3 スロット分解を行わず flat に分解するので、ランタイムに型情報を渡す必要がない。
...
| `declare mathfunc f` + 型注釈 `[f a]` | `ConstantFactor` / `AppliedFactor`(注釈による) | 注釈で確定 |
...
-- 内部表現: instance Ring (Poly Integer [sqrt 2] [β] [γ])
-- ターゲット: Ring (Poly Integer [sqrt 2] [x] [])
-- 単一化: β = [x], γ = []
-- → 解決成功、Poly Integer [sqrt 2] [x] [] が保存
```
...
2. `T` のどこかのスロットに `[..]` がある → 常に適用(原子集合 unknown)
3. 上記以外(全スロット閉じている) → `triggerSymbols(R) ∩ atoms(T) ≠ ∅` のときのみ適用
   - `atoms(T) = cs ∪ ss ∪ fs`(入れ子型は全レベルの和集合)
...
`factor` は `Factor` 型全体のマッチャーで、`constantFactor`, `symbol`, `appliedFactor` の和集合として振る舞う。各分類専用のマッチャー（`constantFactor`, `symbol`, `appliedFactor`）も提供し、`term m` で得た flat モノミアルの各要素を分類したい場合にユーザーが明示的に適用する。
...
3 スロット(cs/ss/fs)への振り分けはマッチャー側では行わない。分類したい場合は、`$mono` の各要素に対して `symbol`/`constantFactor`/`appliedFactor`/`factor` マッチャーをユーザーが適用する。
...
-- Symbol のみを抽出したい場合: multiset + symbol マッチャーで各要素を分類
match a as term integer with
  | ($c, multiset (symbol $name $idx, $exp) $syms) -> ...
  -- syms は (Symbol, Integer) のリスト
  -- symbol マッチャーでないものは除外される
...
`term m` の戻り値は係数と flat モノミアル(`[(Factor, Integer)]`)のタプル。ランタイム Monomial が flat であることを反映し、マッチャーで 3 スロット分解は行わない。
...
- **実装ノート**:
  ...
  ```
- ユーザーが分類したい場合は、得た `$mono` の各要素に対して `symbol`/`constantFactor`/`appliedFactor` マッチャーを適用する(設計判断: マッチャーでの 3 スロット分解はランタイム型情報を必要とし複雑化するため、flat に留める)
