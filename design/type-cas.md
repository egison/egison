...
| **tensor index 評価バグ修正** | `tribonacci.egi`: `Tensor index must be an integer or a single symbol`; `thurston-non-sym.egi`: `Inconsistent tensor index: [_?] vs [_<val>]`. `Core.hs:tref` および `pmIndex`/`pmIndices` (Data/Utils.hs) のパス検証 | 中規模 |
