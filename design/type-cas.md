...
| **多項式 GCD / 共通因子約分** | 分子・分母の多項式 GCD (Euclidean) を実装し、`(c²r-2GM)·X / (c²r-2GM)·Y → X/Y` のような約分を CAS 正規化で fire させる。Schwarzschild の Christoffel 簡約や `T2` の `(b+a cos θ)^k` 次数縮小に効く。Math/CAS.hs の `casNormalizePoly` 系に追加 | 中〜大 |
