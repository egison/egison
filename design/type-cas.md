...
閉じたスロット `[cs] [ss] [fs]` の間では、**同じ原子が複数スロットに現れてはならない**（disjoint 制約）。

```egison
-- NG: sqrt 2 が cs と fs の両方に現れる → 型エラー
def e : Poly Integer [sqrt 2] [] [sqrt 2] := ...

-- OK: disjoint
def e : Poly Integer [sqrt 2] [x] [sin x] := ...
```

理由: `term m` マッチャーの 3 スロット分解や coerce 検証は、原子を `cs`/`ss`/`fs` 集合にルックアップして振り分ける。disjoint でないと振り分け先が一意に決まらない。

開いたスロット `[..]` を含む場合は disjoint 制約は不要（`[..]` は任意の原子を受け入れるワイルドカード）。
...
- 部分型関係: `Poly a [cs] [ss] [fs] ⊂ Poly a [..] [..] [..]` が常に成立
- `join` で一方が `[..]` の場合、そのスロットは結果も `[..]`
...
2. 閉じたスロット同士で原子集合が異なる場合は、Embed（coercive subtyping）で各スロット独立に拡大して揃える
...
-- polyAdd の型は Poly a [..] [..] [..] なので、各スロットは any 扱い → そのまま受け入れる
...
```egison
polyJoinAdd :: {ss1, ss2} Poly a [] [ss1] [] -> Poly a [] [ss2] [] -> Poly a [] [ss1 ∪ ss2] []
```

##### `[..]` 採用の理由

HM 互換で `[cs₁ ⊆ cs₂]` のような集合包含制約付き単一化を導入せずに済む。`[..]` を any にすることで:
...
**`Monomial` は型レベルでのみ 3 スロット（定数 Factor / 平のシンボル / 合成 Factor）に分かれ、ランタイムでは flat な `(原子, 冪指数)` のリスト**として保持する。型の `[cs] [ss] [fs]` は「この原子集合がこのスロットに属することを型レベルで表明する」契約であり、ランタイム値には分類タグを保持しない。

分類の判定（ある原子がどのスロットに属するか）は、以下の境界操作でのみ必要に応じて行う：

- `coerce : MathValue → Poly a [cs] [ss] [fs]`（所属検証）
- `Differentiable MathValue` のディスパッチ（連鎖律の要否判定）
- `term m` マッチャーによる 3 スロット分解（`($c, ($mc, $ms, $mf))` パターン）

`casNormalize` の内部ループでは分類判定を**行わない**（正規化順序に分類が依存する問題を回避）。詳細は「境界での分類判定」セクションを参照。

以下の内部表現例では、各原子がどのスロットの意味で使われているかをコメントで示すが、ランタイムの `Monomial` は flat な単一リストである：
...
Poly Integer [sqrt 2] [x] []
                              → CASPoly [CASTerm (CASInteger 1) [(sqrt 2,1),(x,1)],
                                 ...
                                 -- 型レベルで sqrt 2 は cs, x は ss に属するが、
                                 -- ランタイムは同じ flat リストに混在
Poly Integer [] [] [(sin x)]
...
-- 型レベルの 3 スロット (cs/ss/fs) はここでは保持しない
-- 冪指数は負も可（ローラン多項式のため）
...
**ランタイムを flat にする設計判断**: 型レベルでは `Poly a [cs] [ss] [fs]` の 3 スロットが明確な意味を持つ（微分で閉じる条件、embed の対象選択、coerce の検証など）。しかしランタイム表現で 3 スロットに分けると、注釈のない中間値（`MathValue` レベルの算術）で「どのスロットに入れるか」がランタイムに決められなくなる問題が生じる（`ConstantFactor` と `AppliedFactor` のランタイム表現が同一のため）。

flat に保持することで:

- `casPlus`/`casMult` のモノミアル結合が一意に決まる（同じ原子は常に同じ表現）
- `casNormalize` のループ内でスロット分類を判定する必要がない
- embed（`Factor → Poly`, 原子集合拡大 `Poly [cs] → Poly [cs']` 等）の多くがランタイム no-op になり、coherence が自明に成立

分類が必要なのは以下の**境界操作**のみ（次節で詳述）:

1. `coerce : MathValue → Poly a [cs] [ss] [fs]` の所属検証
2. `Differentiable MathValue` のディスパッチ（連鎖律の要否）
3. `term m` マッチャーの 3 スロット分解

これらはワンショット操作であり、正規化ループの内側で走らないので、「`sin(x-x)` の正規化で分類が変わる」問題（L468 参照）は起きない。

### 境界での名前集合照合

ランタイム分類関数（`classifyFactor` や `hasFreeSymbol` 等の自由シンボル解析）は**持たない**。代わりに、`Poly a [cs] [ss] [fs]` の閉じた型が指定された境界で、**型注釈に現れる原子の名前集合との照合**を行う。

#### 設計原則

- 原子の CF / S / AF 分類は**ユーザーの型注釈が決定**する。ランタイム解析はしない
- 型注釈に書かれた原子の名前集合 `cs ∪ ss ∪ fs` は **disjoint**（同じ原子が複数スロットに現れてはならない。違反時は型エラー）
- 従って、ある原子が型注釈上のどのスロットに属するかは、注釈時点で一意に決まる
- 境界操作では「この原子は `cs ∪ ss ∪ fs` に含まれるか」「含まれるならどのスロットか」を注釈集合のルックアップで決める
...
```haskell
-- Poly a [..] [..] [..] → Poly a [cs] [ss] [fs] の検証
coerceToClosed :: [SymbolExpr] -> [SymbolExpr] -> [SymbolExpr]
               -> CASValue -> Either CoerceError CASValue
coerceToClosed cs ss fs (CASPoly terms) = do
  let union = cs ++ ss ++ fs
  forM_ terms $ \(CASTerm _ mono) ->
    forM_ mono $ \(sym, _) ->
      require (sym `elem` union)  -- 注釈された原子のみ許可
```

スロット別の検査はしない。ユーザーが `[sqrt 2]` を cs に書くか fs に書くかはユーザーの責任で、システムは**どこかに書かれていること**だけを検証する。

**2. `term m` マッチャーの 3 スロット分解**:

型注釈 `Poly a [cs] [ss] [fs]` の cs / ss / fs 集合を使って、flat Monomial の各原子をルックアップで振り分ける:

```haskell
termMonoConsts  cs _  _  (CASTerm _ m) = [p | p@(sym, _) <- m, sym `elem` cs]
termMonoSymbols _  ss _  (CASTerm _ m) = [p | p@(sym, _) <- m, sym `elem` ss]
termMonoComps   _  _  fs (CASTerm _ m) = [p | p@(sym, _) <- m, sym `elem` fs]
```

cs / ss / fs が disjoint なので、各原子は最大 1 つのリストに入る。

**3. `Differentiable MathValue` のディスパッチ**:
...
引数が定数（自由シンボルなし）の場合、連鎖律の内側微分 `∂/∂ arg s` が自然に 0 を返すので、結果も 0 になる（`sqrt 2`, `log 3` 等の「実質 CF」のケース）。従ってランタイム分類なしで数学的に正しい微分が得られる。
...
| `Frac (Poly a [cs] [ss] [fs])` | `CASFrac (CASPoly []) (CASPoly [CASTerm <one> []])` （空 Monomial = 空リスト） |
...
不定元（`x`, `y`）とシンボル（`i` 等）は型システム上の区別がない。どちらも `declare symbol` で宣言し、`Poly a [cs] [ss] [fs]` の `ss` スロットに入る。簡約規則は `declare rule` で別途付与する（例: `declare rule auto term i^2 = -1`）。
...
`Factor` は原子的な数式要素全体を指す型で、**微分に対する挙動** により 3 つの真部分型に分かれる:

| 分類 | 例 | 性質 |
|----|----|----|
| **`ConstantFactor`**（定数 Factor） | `sqrt 2`, `log 3`, `sin (1+1)` | 任意のシンボル `s` で微分すると 0（連鎖律対象が存在しない） |
| **`Symbol`**（平のシンボル） | `x`, `y`, `r`, `θ` | `s` 自身で微分すると 1、それ以外は 0 |
| **`AppliedFactor`**（合成 Factor） | `sin x`, `sqrt x`, `log (x+1)` | 連鎖律で新しい Factor が生成されうる（微分で型構造が変わる可能性） |

分類の判定は **ユーザーの型注釈で行う** （「静的な分類」参照）。典型例では「内部に自由シンボルを含むかどうか」が判断基準になるが、それを自動判定する機構は持たない。
...
Factor 値の 3 分類は **ユーザーが型注釈で明示する** ことで静的に決まる。自動判定（`freeSymbols` による内部解析）は採用しない。理由:

- `sin (x - x)` のように、引数が正規化で `0` に簡約されるケースで「分類が正規化順序に依存する」問題を避ける
- 実装がシンプルになる（`freeSymbols :: SymbolExpr -> Set Symbol` が不要）
- ユーザーが数式の扱いたい形を型で明示する CAS 設計の思想とも整合

```egison
declare symbol x, y

-- Symbol は declare symbol 経由で確定
x  : Symbol
y  : Symbol

-- declare mathfunc 経由の関数適用は、デフォルトでは MathValue。具体的な分類は注釈で絞り込む
(sqrt 2 : ConstantFactor)
(sqrt x : AppliedFactor)
(sin (1 + 1) : ConstantFactor)
(log (x + 1) : AppliedFactor)
```

#### 宣言ルートと分類の対応

| 宣言ルート | デフォルト分類 | 注釈による絞り込み |
|---|---|---|
| `declare symbol x` | `Symbol`（確定） | 不要 |
| `declare symbol sin x`（合成原子の登録） | `AppliedFactor`（確定） | 不要 |
| `declare mathfunc f ...` の適用 | `MathValue`（最汎） | `(f arg : ConstantFactor)` 等で絞り込む |

#### 分類注釈の情報量

`ConstantFactor` / `AppliedFactor` の注釈は「Factor 3分類のどれか」を示すだけで、**具体的な原子（例: `sqrt 2`）は型に現れない**。原子情報まで型で保証したい場合は、`Poly` / `Term` のフル型を使う:

```egison
(sqrt 2 : ConstantFactor)                       -- 単一の定数Factor、原子不明
(sqrt 2 : Poly Integer [sqrt 2] [] [])          -- 原子が sqrt 2 確定、単項、微分で閉じる
(sqrt 2 : Term Integer [sqrt 2] [] [])          -- 単項であることも型で表明
(sqrt 2 : ConstantPoly Integer [sqrt 2])        -- 上記の糖衣
```

実用的には下3つ（閉じた `Poly`/`Term`）を推奨し、`ConstantFactor` 注釈は **後続で `Poly` に embed される前提の中継用** として使う。

### 内部表現

3分類は**型レベルでのみ区別**し、ランタイム表現はいずれも `CASFactor SymbolExpr` で統一する。

```
Symbol:         CASFactor (Symbol _ _ _)
ConstantFactor: CASFactor (Apply1 _ _) or CASFactor (Quote _)
AppliedFactor:  CASFactor (Apply1 _ _) or CASFactor (Quote _)
```

`ConstantFactor` と `AppliedFactor` はランタイム表現が同じで、**型注釈による静的区別のみ**。これにより:

- `casNormalize` の内部ループでは分類判定を行わない
- `Poly` に embed しても内部表現は変わらず、Monomial に flat に追加される（スロット分離はしない）
- **ランタイムの CF/AF 分類関数（`classifyFactor` や `hasFreeSymbol`）は一切持たない**。ユーザーの型注釈を信頼する
- coerce は「原子が `cs ∪ ss ∪ fs` に含まれるか」だけを検証（スロット別の分類チェックはしない）
- `term m` マッチャーの 3 スロット分解は、静的型情報の `cs`/`ss`/`fs` 名前集合で振り分ける
- `Differentiable MathValue` は **always-AF 規則**（Apply1 は常に連鎖律、引数が定数なら自然に 0 になる）で動作
- 詳細は「構成的な内部表現 > 境界での名前集合照合」を参照

### Factor の各分類から Poly への埋め込み

Factor 値が `Poly` に embed されるとき、ランタイム表現はいずれも Monomial の末尾に `(sym, 1)` を追加する操作になる（flat 表現のため、スロットによる振り分けはしない）。型レベルでは対応するスロットに原子が加わったことを表明する:

```
ConstantFactor → Poly Integer [cs] [] []     -- 型: cs に原子が追加
Symbol      → Poly Integer [] [ss] []     -- 型: ss に原子が追加
AppliedFactor  → Poly Integer [] [] [fs]     -- 型: fs に原子が追加
-- ランタイムはいずれも Monomial に (sym, 1) を追加するだけ
```
...
- `sqrt n`（`n : Integer`）の結果は `isPerfectSquare n` の実行時判定で `Integer`（`sqrt 4 = 2`）にも `ConstantFactor`（`sqrt 2`）にもなり得る。実行時値に依存する返り値型を静的に決めるには依存型が必要
...
呼び出し側で具体的な型が必要な場合は、**式レベル型注釈**（後述）で `coerce` を挿入する。注釈の粒度は以下の2段階:

```egison
-- 注釈なし(MathValue のまま)
sqrt 2                                          -- : MathValue

-- 分類のみ(ConstantFactor / AppliedFactor、原子は型に現れない)
(sqrt 2 : ConstantFactor)                       -- : ConstantFactor
...
(sqrt m : Poly Integer [..] [] [])             -- cs スロットは any(具体原子は静的に不明)
```

### 数学関数の適用規則 (`declare apply`)

`declare mathfunc` で宣言した関数の適用時の評価ロジックを `declare apply` で定義する。引数が具体的な値の場合に簡約を行う。RHS にはユーザー定義関数を含む任意の Egison 式を書ける。返り値型は `MathValue`（実行時の値によって実際の型が異なる）。
...
```egison
def s := (sqrt 2 : ConstantFactor)  -- s : ConstantFactor（式レベル型注釈で coerce）
...
-- 2. 適用規則（アルゴリズム的簡約、各ブランチに式レベル型注釈で実装検証）
declare apply sqrt x :=
  if isInteger x then
    if isPerfectSquare x then (isqrt x : Integer)
    else if hasPerfectSquareFactor x then
      let (k, m) = extractPerfectSquareFactor x in
      (k * sqrt m : Poly Integer [..] [] [])      -- cs スロット非空の単項(原子は実行時依存)
    else ('sqrt x : Poly Integer [..] [] [])       -- 'sqrt x は単一定数Factor(cs に1原子)
  else ('sqrt x : MathValue)                       -- 引数が非整数ならフォールバック

-- 3. パターン書き換え規則
declare rule auto term (sqrt $x)^2 = x
```

**ブランチ別の式レベル型注釈**: 上記の `: Integer` や `: Poly Integer [..] [] []` は既存の式レベル型注釈 `(expr : Type)` の再利用。新構文は不要で、各ブランチの実装者意図を**型チェッカーが検証**する。全ブランチの型を join した結果が関数の返り値型 `MathValue` と整合する必要がある。

閉じたスロット `[sqrt m]` は **`m` が実行時値なので書けない**（HM 非互換）。この場合は `[..]` で退行する。原子集合まで静的に指定したい場合は呼び出し側で注釈する。

呼び出し側で式レベル型注釈を付けることで `coerce` が自動挿入され、具体的な型に絞り込める:

```egison
(sqrt 4 : Integer)                         -- 2 : Integer
(sqrt 8 : Poly Integer [sqrt 2] [] [])     -- 2 * sqrt 2 : Poly Integer [sqrt 2] [] []
(sqrt 2 : ConstantFactor)                  -- sqrt 2 : ConstantFactor（原子不明）
...
-- 各分類の Factor は、ランタイムでは同じ形の CASPoly を生成する
-- スロット区別は型レベルのみ。Monomial は flat リストに (sym, 1) が入るだけ
...
(sqrt 2 : ConstantFactor)                   -- sqrt 2 の結果を ConstantFactor に coerce
...
| `ConstantFactor` | 単一の定数Factor（原子不明） | 中継用（後続で `Poly` に embed される前提） |
| `AppliedFactor` | 単一の合成Factor（原子不明） | 同上 |
| `Poly Integer [..] [] []` | cs のみ非空、**微分で閉じる**、具体原子は不明 | 実行時値を含む場合の退行形 |
| `Poly Integer [sqrt 2] [] []` | 原子が `sqrt 2` 限定、`(sqrt 2)^2 = 2` 等のルールが型上で追跡可 | **推奨**（引数が静的に既知の場合） |
| `ConstantPoly Integer [sqrt 2]` | 上記の糖衣 | 同上 |
| `Term Integer [sqrt 2] [] []` | 単項であることも表明 | さらに精密に絞り込みたい場合 |

`ConstantFactor` / `AppliedFactor` 注釈は Factor 3分類情報しか持たないため、**最終的な型注釈としては情報不足**（例: `(sqrt 2 : ConstantFactor) + 1` の結果は `Poly Integer [..] [] []` 相当で、原子 `sqrt 2` が型に現れない）。具体的な原子を含む式に対しては、**閉じた `Poly` 型を直接注釈**することを推奨する:

```egison
-- 推奨: 閉じた Poly で注釈(原子情報が型に残る)
def s : Poly Integer [sqrt 2] [] [] := sqrt 2

-- 非推奨: ConstantFactor だと後続計算で原子が追跡できない
def s : ConstantFactor := sqrt 2          -- 動くが、s + 1 の型が Poly Integer [..] [] [] に落ちる
```
...
| 変換 | 検証内容 |
|------|----------|
| `Poly a [..] [..] [..] → Poly a [cs] [ss] [fs]` | 全項のモノミアルの各原子が、`cs ∪ ss ∪ fs` の名前集合に含まれるか（スロット別の分類チェックはしない、ユーザーの注釈を信頼） |
| `Poly a [..] [..] [..] → Integer` | 全項のモノミアルが空で、係数が `Integer` に変換可能か |
| `Poly a [..] [..] [..] → ConstantFactor` | 単一項で係数が1、Monomial が単一原子の1乗（分類は注釈で CF として表明） |
| `Poly a [..] [..] [..] → Symbol` | 単一項で係数が1、Monomial が単一原子の1乗で、その原子が `Symbol` コンストラクタ（これのみ構造的に判別可能） |
| `Poly a [..] [..] [..] → AppliedFactor` | 単一項で係数が1、Monomial が単一原子の1乗（分類は注釈で AF として表明） |
| `Frac a → a` | 分母が1か |

> 原子の CF / S / AF 分類はユーザーの型注釈で決まる。coerce は注釈された名前集合との照合のみを行う（詳細は「構成的な内部表現 > 境界での名前集合照合」参照）。
...
-- 閉じた Poly と埋め込み可能な型
join(Poly a [cs] [ss] [fs], b) = Poly (join(a, b)) [cs] [ss] [fs]    -- b が a に embed 可能
join(a, Poly b [cs] [ss] [fs]) = Poly (join(a, b)) [cs] [ss] [fs]    -- a が b に embed 可能
...
> 各スロットの和集合は **スロット内の原子集合に対して** 計算する。スロット間で原子は混ざらない。

> あるスロットが `[..]` のとき、そのスロットだけ開いた型として吸収される。
...
p + q
-- joinTypes で Poly Integer [] [x, y] [] を計算（ss の和集合）
-- 双方を embed
...
CAS の演算で原子集合（`cs`/`ss`/`fs`）を静的に保存したいかどうかは、**用途ごとに異なる**。設計はこれを 2 つの機構で表現する:
...
型クラスのインスタンスヘッドにある `[..]` は **パラメトリック（型変数として束縛）** として扱われ、解決時に具体的な `[cs]` `[ss]` `[fs]` に特化される。これにより、型クラス経由の演算は原子集合を自然に保存する。
...
ただし、**静的型保存のためのインスタンス特化は必要**: `Ring (Poly a [..] [..] [..])` のような汎用インスタンスのヘッドにある `[..]` はパラメトリックな型変数として扱われ、具体的な `[cs]` `[ss]` `[fs]` への解決時に自動的に特化される（型レベルの特化）。これにより `a + b : GaussianInt` のように結果の型が保存される。ユーザーが `instance Ring GaussianInt` を明示的に書く必要はなく、汎用テンプレートから解決機構が自動派生する。
...
instance {Differentiable a} Differentiable (Term a [cs] [ss] [fs]) where
  ∂/∂ t s = ...  -- 係数の微分 × モノミアル + 係数 × モノミアルの微分（product rule）
                 -- モノミアルは flat リスト。各原子 (sym, e) の分類は型注釈 cs/ss/fs の
                 -- 名前集合で判定: cs → 冪則の内側微分が 0 なので項全体 0、
                 -- ss → 冪則、fs → 連鎖律
                 -- ユーザーが sin x を cs に分類した等の誤注釈はそのまま反映（信頼）
...
**always-AF 規則の正しさ**: `Apply1 f a` を常に連鎖律で扱うと、`a` が定数（自由シンボルなし）の場合でも `∂/∂ a s` が再帰的に 0 を返すため、結果は `f'(a) * 0 = 0` となり、数学的に正しい。従って `sqrt 2` や `log 3` のような「実質 CF」も正しく 0 に微分される。これによりランタイムの `classifyFactor` / `hasFreeSymbol` 解析は不要になる。
...
> **設計方針（3スロット型 + flat ランタイム）**: 3 スロット (`cs` / `ss` / `fs`) は**型レベルのみ**に設け、ランタイムの `Monomial = [(SymbolExpr, Integer)]`（flat な単一リスト）は現状のまま維持する。これにより Phase 1 で確立した `casPlus`/`casMult`/`casNormalize` のロジックを変更する必要がなくなる。詳細は「構成的な内部表現 > Haskell での表現」および「境界での分類判定」を参照。

### Phase 1.5: SymbolSet の SymbolExpr 化（縮退版）

当初案の「3 スロット化リファクタリング」は廃止。ランタイム分類関数（`classifyFactor` / `hasFreeSymbol`）も持たない。型注釈との照合に必要な最小限の変更のみ行う。

- [ ] `SymbolSet` の `SymbolSetClosed [String]` を `SymbolSetClosed [SymbolExpr]` に変更（正規化済み `SymbolExpr` の集合に）
- [ ] パーサーで `Poly Integer [sqrt 2, sin x] [..] [..]` のような注釈をパースする際、各エントリを正規形の `SymbolExpr` に変換する
- [ ] `SymbolExpr` の `Eq` インスタンスが構造的比較で動作することを確認（照合に使うため）
- [ ] 型チェッカーで `cs`/`ss`/`fs` の disjoint 制約を検証（違反時は型エラー）
- [ ] 名前集合メンバーシップ関数（`SymbolExpr -> [SymbolExpr] -> Bool`）を実装
- [ ] `casPlus`/`casMult`/`casNormalize` は**変更不要**（flat Monomial のままで OK）
- [ ] mini-test: disjoint 違反の型エラー、正規形比較のテスト

以下の項目は当初案から削除:

- ~~`Monomial` を 3 スロット構造に変更~~ → **不要**（flat を維持）
- ~~`casNormalizePoly`, `casTermsGcd`, `monoGcd` を 3 スロット対応に~~ → **不要**
- ~~`casPlus`, `casMult` のモノミアル比較・結合ロジックを 3 スロット独立に~~ → **不要**
- ~~`classifyFactor` / `hasFreeSymbol` の実装~~ → **不要**（ランタイム分類を持たない方針）
...
-- ランタイム表現は同一(CASFactor (Apply1 _) / CASFactor (Quote _))
-- 型注釈で静的に区別されるのみ。マッチャーは静的型情報に基づき適切に選ぶ
...
`constantFactor` / `appliedFactor` / `symbol` は、`term` マッチャーの 3 スロット分解が返す 3 つのリストそれぞれの要素にマッチする目的で使う。ランタイム Monomial は flat であり、`term` マッチャーは静的型情報から得た `cs`/`ss`/`fs` の名前集合を使って振り分ける（「構成的な内部表現 > 境界での名前集合照合」参照）。既存の `PDSymbolPat`, `PDApply1Pat`, `PDQuotePat` に対応するプリミティブ関数を使う。
...
- **実装ノート**: ランタイム Monomial は flat（`[(SymbolExpr, Integer)]`）なので、`termMonoConsts` / `termMonoSymbols` / `termMonoComps` は**マッチの静的型情報（`cs`/`ss`/`fs` の名前集合）を受け取り**、原子を集合ルックアップで振り分ける:
  ```haskell
  -- cs, ss, fs は型注釈由来の SymbolExpr 集合。Monomial 中の各原子を集合メンバーシップで振り分ける
  termMonoConsts  cs (CASTerm _ m) = [p | p@(sym, _) <- m, sym `elem` cs]
  termMonoSymbols ss (CASTerm _ m) = [p | p@(sym, _) <- m, sym `elem` ss]
  termMonoComps   fs (CASTerm _ m) = [p | p@(sym, _) <- m, sym `elem` fs]
  ```
  `cs`/`ss`/`fs` は disjoint なので、各原子は高々 1 つのリストに入る。型が `Poly a [..] [..] [..]` で名前集合が不明な場合、`term m` マッチャーの 3 スロット分解パターンは使えない（型エラー）。`assocMultiset factor` パターンで全原子を一括して取り出す必要がある。
- [ ] `term` マッチャー関数を `lib/math/expression.egi` に定義
- [ ] `termCoeff`, `termMonoConsts`, `termMonoSymbols`, `termMonoComps` のプリミティブ関数追加（名前集合を引数に取る）
...
- 基本インスタンスの実装（`Math/CAS.hs` にランタイム検証関数）
  - `Coerce (Poly a [..] [..] [..]) (Poly a [cs] [ss] [fs])` — 各原子が `cs ∪ ss ∪ fs` の名前集合に含まれるかを検証（スロット別分類はしない）
  - `Coerce (Poly a [..] [..] [..]) Integer` — 全モノミアルが空で定数項のみかを検証
  - `Coerce (Poly a [..] [..] [..]) ConstantFactor` — 単一項で Monomial が単一原子1乗かを検証（CF としての分類はユーザーの型注釈で表明）
  - `Coerce (Poly a [..] [..] [..]) Symbol` — 単一項で Monomial が単一原子1乗、その原子が `Symbol` コンストラクタかを検証
  - `Coerce (Poly a [..] [..] [..]) AppliedFactor` — 単一項で Monomial が単一原子1乗かを検証（AF としての分類はユーザーの型注釈で表明）
