...
### G6 追補 2: 項単位トリガーガード (2026-07-07)

- **実装**: パターン規則の desugar (buildPatternRuleBody) が生成する
  一段ステップに、`casContainsAnySymbol` (新プリミティブ、短絡走査) による
  ガードを挿入 — トリガー名をひとつも含まない部分値ではマッチャーを
  起動しない。ANY-of 意味論で健全 (LHS がマッチしうる部分値は必ず
  パターンの頭シンボルを含む; トリガー集合の余分な要素は通過を増やす
  だけ)。空トリガーの規則はガードなし (常時実行のまま)。
- **正直な計測**: 「素の多項式 36 項+log 原子 1 個」(値のトリガーが log のみ)
  で 6.64 → 6.08s (**約 8%**)、exp 混在形で 13%。当初「80/20」と見積もったが
  過大 — これらの負荷では多項式演算そのものが支配的で、ガードが削れるのは
  「トリガーを含まない項へのマッチ試行」だけ。ガード自体はマッチャー起動より
  常に安いので負ける形状はない (保持)。
- **残る本丸**: トリガーを**含む**項への正当なマッチ試行の単価
  (マッチャーの起動コスト)。削るにはマッチャーの特殊化 (単一因子 LHS の
  直接判定) や規則の頭シンボル索引 (discrimination net) が要る — 据え置き。

- **残り (このフェーズのスコープ外)**: 規則マッチャーの特殊化/索引 (上記)。
