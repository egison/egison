...
- **GF(p^k) (G8、§3.8 に設計)** — GB エンジンの係数**体演算 (reduce, divide)** の
  パラメータ化 + cas-quotient との合成。reduce だけでは不十分 (monic 化と
  単項式除算が係数除算 = モジュラ逆元を要求) という精密化済み。
...
### 3.8 GF(p^k) — GB エンジンの係数体パラメータ化 (G8 構想 — 2026-07-07 設計精密化、未着手)
...
| **G7** | **根号原子の分枝追跡** (§3.7): ペア積関係式の数値分枝証明書つき添加 + `idealNF` 明示適用。検収 = 4 項 root-of-unity `z₄⁵ = 1` | 中 | G2, G5 |
| **G8** | **GF(p^k)** (§3.8): GB エンジンの係数体演算 (reduce, divide) パラメータ化 + cas-quotient との合成。検収 = GF(4) デモ | 中 | G2, 商 q1–q4 |
