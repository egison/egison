...
分数どうしの加算（`casPlus'`）は、二つの分母を先に GCD で割ってから最小公分母の上で足す
（`reducedDenominators`）。分母の積で足してから約分する方式では、GCD が扱えない値（平方根の原子を
含む分子など）で約分が起きず、加算のたびに分母の次数が倍になっていた。

平方根の原子は分母に残さない（`Math/Rewrite.hs` の `casRewriteSqrt`）。`(sqrt a)^n` は n ≠ 1 の
すべてで被開平数を外に出し、負の冪は `(sqrt a)^-1 = sqrt a / a` と有理化する。分母の全ての項が共有する
平方根の原子は、分子分母にその原子をかけて被開平数に置き換える。これにより分母は通常の記号の多項式に
なり、上の GCD が機能する。分母が a + b·sqrt c の形のときの共役による有理化は未実装である。
