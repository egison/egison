...
- `trigIdeal θ` は角 θ のピタゴラス生成元リストを返すだけの 1 行ヘルパ。
  **生成元を素の式で書くと既存の自動ピタゴラス規則に食われて 0 になる**
  ...
  ため、非正規化コンストラクタ (`'f` / `^'` / `+'` / `-'` — 規則 RHS が無限ループ
  回避に使うのと同じ既存イディオム) で組む。**現行の処理系でそのまま動く**:
  ...
  `declare ideal` は構文フォームなので AST から直接生成元を作れてこの問題と無縁
  (declare rule RHS と同じ `unNormalizeOps` の扱いを生成元に適用する)
