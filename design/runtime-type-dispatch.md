...
> **2026-07-05 更新**: 順序の実体は旧 `Type/Join.hs:isSubtype` (部分骨格) から
> `Type/Subtype.hs:isSubtypeWith` (完全骨格 + `declare cas-subtype` 宣言辺) に
> 一本化された。完全骨格では MathValue 係数のとき `Poly MathValue [..]` と
> `Frac MathValue` のような**順序同値対** (相互埋め込み = 同じ値集合の異なる
> 正規形) が生じ最特異が一意に決まらないため、最特異候補が複数残った場合は
> **target の頭部構成子と一致する候補を選ぶ** (`sameCasHead` tie-break —
> 「型は表現を選ぶ」の設計原理どおり、dispatch は target 自身の表現に従う)。
> それでも決まらなければ従来どおり ambiguity。

CAS 型の subtype 関係 (`Type/Subtype.hs:isSubtypeWith`) を活用:
