...
- `Embed` typeclass と `coerce`/`coerceToX` 関数群は **Phase C (2026-05-07) で削除**。widening/narrowing は完全に reshape 機構に統合された。
...
- 2026-05-07: Phase C 完了。`Embed` typeclass + 全 instance、`def coerce` / `coerceToInteger` / `coerceToFracInteger` / `coerceToPolyInteger` / `coerceToPolyFracInteger` / `coerceToFracPolyInteger` / `coerceToFactor` / `coerceFracToInteger` / `coercePolyToInteger` を `lib/core/base.egi` から削除。`primCoerce` プリミティブと `coercePrim` 関数を `Primitives.hs` から、型登録を `Type/Check.hs` から削除。テスト書き換え: `def x : T := embed e` → `def x : T := e`、`def x : T := coerceToX e` → `def x : T := e`、standalone `(coerceToInteger 5)` → def boundary に置換。72/73/78/81/88/92/104/107 各テストを更新。`75-cas-subtype-unify.egi` のコメントを更新。cabal test 21/21 PASS、mini-test 75/75 PASS、warnings 0 件。これで widening/narrowing は単一の reshape 機構 (型注釈 → elaboration → IReshape AST node → casReshapeAs) に統一された。
- 2026-05-07: Phase E (テスト・検証) 完了。包括テスト `mini-test/109-reshape-comprehensive.egi` (22 assertion) を追加。検証項目: (1) 内側 typeclass dispatch の保持 (`def n : Integer := 1+2+3`、`def a : Frac Integer := (1/2)*(1/3)` 等 — 旧 A1 案で壊れていた構造)、(2) runtime 構造の書き換え (Frac→Integer collapse、Poly cancel)、(3) 非 CAS 型の no-op (Bool/String/Char)、(4) `MathValue` 注釈は `casNormalize` 経由、(5) 内側型引数への再帰 (Poly (Frac Integer) の係数 reshape、Frac (Poly Integer) の num/denom)、(6) 多相 def と関数型 def の no-reshape、(7) Factor 型と Poly への widening。全 22 assertion PASS、cabal test 21/21、mini-test 76/76 PASS、Math/Rewrite.hs の Sweet Egison quasi-quoter 由来 warning のみ (reshape 無関係 pre-existing)。**A1 干渉問題が再発していないことを実証済み**。既知の制限: `def n : T := zero` (引数なし typeclass method + 型注釈) が "Expected function, but found: 0" でエラー — reshape 無効化しても同症状なので reshape とは無関係の既存 issue。Phase D で要対処。
- 2026-05-07: Phase D 完了。`(e : T)` 式レベル型注釈の reshape 化。実装: (1) `Parser/NonS.hs` の `tupleOrParenExpr` を拡張し `try typeAnnotated <|> tupleOrSection` で `(e : T)` を `TypeAnnotation Expr TypeExpr` AST node として parse。(2) `Desugar.hs` の `desugar (TypeAnnotation expr typeExpr)` を `IReshape (typeExprToType typeExpr) (desugar expr)` 生成に変更 (旧: 注釈を捨てる)。(3) `Type/Infer.hs` の `inferIExprWithContext` に `IReshape` ケースを追加: 内側式の型を inference し、注釈型と subtype unify、`TIReshape finalTy innerTI` を返す。(4) `applySubstToTIExprNodeWithClassEnv` に `TIReshape` traversal を追加。(5) `Type/TypeClassExpand.hs` の `expandTIExprNode` と `Type/TensorMapInsertion.hs` の `insertInNode` にも `TIReshape` traversal を追加。テスト `mini-test/110-reshape-expr-annot.egi` (12 assertion): 単独 `(5 : Integer)`、ネスト注釈 `((1+2) : Integer) : MathValue`、算術内部 `(5 : Integer) + 3`、Poly 注釈、非 CAS 注釈 (Bool/String) 等を検証。全 PASS、cabal test 21/21、mini-test 77/77 PASS。これで型注釈が def boundary 限定でなく任意の式位置で reshape を駆動できるようになった。今後 (c) 関数引数・match 等への拡張は別タスク。
