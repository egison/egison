...
| `Type/EtaExpansion.hs` | Eta展開（未統合。将来Phase 7の最初に実行予定） | `etaExpandTITopExpr` |
...
**EtaExpansion (未統合)**:
`EtaExpansion.hs` は型クラスメソッドの eta 展開（`(+)` → `\x y -> (+) x y`）を行うモジュールとして設計されているが、現在はパイプラインに組み込まれていない。Phase 7 の最初に実行する想定。
...
`Type/EtaExpansion.hs` は型クラスメソッドのeta展開を行うモジュールとして設計されているが、現在はパイプラインに組み込まれていない。将来的にPhase 7の最初のステップとして統合予定。
