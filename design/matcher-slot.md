...
- **A の本体内保留 = 未注釈の内部再帰マッチャー(これは rigid 変数に近い)**。`algebraicDataMatcher` は
  `ILetRecExpr [(_11, matcher)] _11` を生成し、内部で `match (val,tgt) as (_11,_11)` を持つ。この `_11` は
  **宣言型注釈の無い**内部 letrec 束縛なので型が fresh 変数のまま(`poly` 等のトップレベル定義と違い、解決する
  宣言型が無い)。構造検査時に真に多相な `something` と区別できず誤拒否するため、マッチャー本体内では match-site
  検査を保留している。解消法: (i) rigid/skolem 変数、または (ii) `algebraicDataMatcher` の内部マッチャーに結果型を
  与える。
