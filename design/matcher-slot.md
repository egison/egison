...
- **本体内 match-site 検査(保留を撤去 — ユーザ要求「きちんと型検査」)**: 以前は
  ...
  **genuine に型検査される**。フラグ自体は残すが、現在は Coverage 警告の抑制のみに使う。
  ...
  - **既知の狭い false-positive(非作法な明示 self-match-site 再帰)**: concrete 型上の手書き再帰マッチャーが、
    再帰を **next-matcher スロットでなく明示的 match-site**(`match v as self`)で行うと誤拒否される。例
    `def nat : Matcher Nat := matcher | suc $ as (nat) with | $tgt -> match tgt as nat with | suc $n -> [n] …`。
    self-ref `nat` の inner 型が fresh 変数 `t4` になり、`MatcherSlot Nat Nat` への片方向 `⊑` 検査(matcher 側 rigid)
    が `t4` を bind できず拒否する。**作法的な再帰(`algebraicDataMatcher`、`suc nat` のような next-matcher スロット)
    は通る**(再帰型 Nat の `algebraicDataMatcher` は strict mode でも 0 type-errors、実測)。実コードはこれを使う
    ため影響なし。
  - **根本原因(特定済)**: `typeExprToType (TEVar name) = TVar (TyVar name)`(`Type/Types.hs`)が、**bare な型名を
    大文字でも無条件に型変数化**する。よって注釈 `Matcher Nat` が `∀Nat. Matcher Nat`(`Nat` が型変数)に一般化され、
    トップレベル定義の self-ref がそれを instantiate して fresh inner `Matcher t4` を得る(probe で確認:
    `nat scheme = Forall [TyVar "Nat"] (TMatcher (TVar (TyVar "Nat")))`)。`def x : Color := Red` が
    `Red : Integer` になる現象も同根。
  - **fix 試行 → 撤回**: 「大文字始まりの bare 型名は concrete(`TInductive`/組込)」とする正攻法を実装したところ、
    再帰 self-match-site(`nat`)は通るようになったが、**未宣言の `MathExpr` を柔軟な型変数として使う診断テスト**
    (mini-test 51/52/55/56, `test/lib/core/assoc.egi`; いずれも「Tensor Matcher [Tensor MathExpr] issue」の隔離テスト)
    を破壊した。`MathExpr→TMathValue` マップで一部は回復したが、51/52 は**既知未解決の CAS matcher 問題**を露呈した。
    narrow な false-positive を直すために未解決の CAS 問題を引き込む blast radius は過大なので**撤回**。完全解には、
    `typeExprToType` に宣言済み型レジストリを通し「宣言済み inductive 型のみ concrete・未宣言名は別扱い」と
    区別する必要がある(将来課題)。
...
- **残差(論文との微差)**: (1) マッチャー本体内の match-site も**検査される(保留撤去済)**。残る制約は
  **非作法な明示 self-match-site 再帰**(`match v as self` で concrete 型を再帰; 作法的な next-matcher スロット
  `suc nat` や `algebraicDataMatcher` は通る)を誤拒否すること — 根本原因 `typeExprToType` の bare 型名一般化、
  上記参照。(2) `eq`(`{Eq a}=>Matcher a`)は多相変数のため `multiset eq with #concrete` を過剰拒否(論文は
...
- **A の本体内 match-site = 検査される(保留撤去済)。残る制約は非作法な明示 self-match-site 再帰のみ**。
  ...
  - **残る狭い false-positive = 明示 self-match-site 再帰**: concrete 型を `match v as self` で再帰する手書き
    マッチャー(例 `def nat : Matcher Nat := matcher | suc $ as (nat) with | $tgt -> match tgt as nat with | suc $n -> [n] …`)。
    match-site 経路は**片方向 rigid `⊑`** で、self-ref の fresh inner `t4` を `MatcherSlot Nat Nat` に bind できず拒否。
    根本原因は `typeExprToType (TEVar name) = TVar (TyVar name)`(bare 型名を大文字でも型変数化)で、`Matcher Nat` が
    `∀Nat. Matcher Nat` に一般化され、self-ref が fresh inner に instantiate される(probe 確認:
    `nat scheme = Forall [TyVar "Nat"] (TMatcher (TVar (TyVar "Nat")))`)。実コードは `algebraicDataMatcher`/スロットを
    使うため影響なし。
  - **fix 試行 → 撤回**: 「大文字 bare 型名は concrete」とする `typeExprToType` 修正で `nat` は通るようになったが、
    **未宣言の `MathExpr` を柔軟型として使う診断テスト**(mini-test 51/52/55/56, `test/lib/core/assoc.egi`; 全て
    「Tensor Matcher [Tensor MathExpr] issue」隔離テスト)を破壊し、既知未解決の CAS matcher 問題を露呈したため撤回。
    完全解には `typeExprToType` へ宣言済み型レジストリを通す必要がある(将来課題)。
  - 補足(理論): この体系は HM + subsumption 的な片方向検査 `⊑`(順序依存・matcher rigid)で、純 HM ではない。
    ...
    同様に拒否してしまう — 論文はこれを「再帰マッチャーは宣言型がスコープに入る」前提で回避する。`multiset` 等の
    parameterized 型(`Matcher [a]`)を matchする手書きマッチャーは、パターン型 `[α]` に変数葉があり `⊑` 検査が
    それを吸収するため**完全注釈なしでも通る**。nullary/concrete 型の明示 self-match-site 再帰のみが上の制約に当たる。
