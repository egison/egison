...
- **dp アーム catch-all 検査(Def 4.2 の範囲外、opt-in warning)を追加実装**: Def 4.2 は節の**パターン側**
  (pp/next-matcher/body 返り型)しか制約せず、節内の **primitive data pattern アームの網羅性を要求しない**。
  実行時は、pp がマッチした節の dp アームが全て外れると **graceful なバックトラックではなく
  `PrimitiveMatchFailure`("Primitive data pattern match failed")の評価エラー**になるため、Def 4.2 を全て
  満たし `--type-check-strict` を通過するマッチャーでも実行時エラーに到達できる(例: `$ :: $ as (m, bad2 m)
  with | $x :: $xs -> [(x, xs)]` のみで `_ -> []` を欠く節は、空リストターゲットで crash)。
  - 検査(`Infer.hs` の `IMatcherExpr`、`pdArmsExhaustive`/`pdIrrefutable`、`MatcherDataArmsWarning`)は
    **純構文的な十分条件**: いずれかのアームが irrefutable(`_`・`$x`・irrefutable 成分のタプル)、または
    組み込みの閉じた形を尽くす完全ペア — `[]` アーム + irrefutable 成分の cons/snoc アーム(コレクション)、
    `True` + `False` 定数アーム(Bool、sample/nishiwaki.egi 型の列挙)— があれば網羅とみなす。ユーザー ADT の
    構築子列挙による ML 流網羅性は行わない(型環境がデータ構築子と同じ結果型の関数を区別しないため
    構築子集合を安全に列挙できない)ので、ADT を構築子ごとに列挙し catch-all を置かないアーム集合
    (例: sample/poker-hands-with-joker.egi の `Card $x $y | Joker`)は保守的に警告される
    (stdlib 慣習の末尾 `| _ -> []` / 単独 `$tgt` は常に通る)。
  - ゲートは Coverage と同一(`--matcher-consistency-warnings`、デフォルト OFF、matcher 本体内は抑制)。
    stdlib は全節 catch-all 終端スタイルのため**警告 0 件**(`algebraicDataMatcher` 生成節も常に
    `(PDWildCard, [])` 終端で安全)。検証: mini-test/124。
  - **論文側**: Def 4.2 に対応条件(各節の dp アーム網羅性、または「アーム集合は失敗時 [] を返す全域関数」)
    を追加するかは著者(type-pm-paper 側)の判断待ち。追加されれば hard error 化を検討。
