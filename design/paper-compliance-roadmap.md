...
## 4. 記録: 複数 match site 検査と推論エンジンの健全性修正(2026-06-13)

**発端**: 「`def f m := (matchAll … as m with …, matchAll … as m with …)` のように
λ 束縛 matcher パラメータを複数 match site で使うケースは現在の型規則で大丈夫か」の実機確認。
論文側は Algorithm W Step 3a(「a variable bound to such」)が2回目以降の site を
カバーしており規則上は健全(構造要求は join、ターゲットは等値強制、λ 束縛は単相)。
しかし実装は target 型が衝突する例(`[Integer]` と `[String]`)を**受理**していた。

**根本原因(一般 HM レベルの健全性バグ)**: 兄弟部分式(タプル成分・cons の head/tail 等)を
素の環境で独立推論し、返り値 subst を左バイアス `composeSubst` で合成していたため、
同一型変数への衝突束縛が単一化されず片方が黙って破棄されていた。
最小反例: `def bad x := (x + 1, if x then 1 else 2)` が受理。

**修正(global zonk substitution)**:
- `InferState.inferGlobalSubst` を追加。`unifyTypesWithContext` /
  `unifyTypesWithConstraints` / `unifyTypesWithTopLevel` が (1) 両辺を global で
  zonk してから単一化し (2) 結果を global に合成。後続 site は先行 commitment を
  必ず見るので、衝突は単一化時に検出される。
...
1. **instance 辞書ハッシュはヘテロ**(メソッド型混在+`__super_*` に辞書型;
   例: Ord の compare : a→a→Ordering と (<) : a→a→Bool)。shadowing で偶然通っていた。
   → IDefineMany 内のハッシュ束縛(構築上 instance 辞書のみ)を辞書モード推論:
   値型を相互単一化せず `Hash String v`(v fresh)。EnvBuilder の
   「第一メソッド型近似」の env スキーマは温存(リテラル検査のみスキップ)。
2. **assocMultiset の element-view clause** `| #$x :: $ as (assocMultiset m)` は
   型規則上不可能(`::` の head は matched type [(a,Integer)] からペア型;
   ユーザー側 `#v :: p` の v も同様)。本体は x を key 型 a で使用 → occurs check。
   → clause を削除しペアビュー(`(#v, $n) :: rest`)で代替(lib/core/assoc.egi に
   理由コメント)。sample/chopsticks.egi はローカル定義に同 clause を保持(既存 ❌ のまま、
   permissive fallback で実行は可能)。
3. **let の単相性**: パーサは表層 `let` を常に LetRecExpr に脱糖しており、束縛は
   `Forall [] []`(単相)だった。論文 §3 は let 一般化を明言(matcher polymorphism の
   let-bound 変種も依存)。→ `inferIRecBindingsWithContext`(+ 死経路だが ILetExpr 側も)に
   一般化を実装。制限: (i) 単一変数束縛(PDPatVar)のみ(論文の T-LET 形)、
   (ii) 制約に触れる変数は単相のまま(辞書はトップレベル定義でのみ受け渡されるため、
   制約付きローカル多相は実行時辞書機構を追い越す)、(iii) RHS が matcher リテラルの
   束縛(脱糖が生成する letrec ラップ)は単相のまま(一般化すると var 参照の instantiate が
   clause 木と最終型のリンクを切り、stale type vars を再導入する)。

**論文側**: App C Step 3a′ に複数 site の挙動(join・等値ターゲット・λ単相 vs let一般化)を
明文化(2026-06-13)。それ以外の規則変更は不要(規則は元から正しかった)。

**テスト**: `mini-test/123-multisite-matcher-param.egi`(受理4件: 2site同型、join、
let多相)、`test/type-error/55-multisite-target-conflict.egi`、
`test/type-error/56-multisite-structural-join.egi`。
