...
data SymbolExpr =
    Symbol Id String [Index MathExpr]
  | Apply EgisonValue [MathExpr]
  | Quote MathExpr
  | FunctionData (Maybe EgisonValue) [EgisonValue] [EgisonValue] [Index MathExpr] -- fnname argnames args indices
 deriving (Eq)

type Id = String

deriving instance Eq MathExpr
...
deriving instance Eq SymbolExpr

--
-- Scalars
--

symbolMathExpr :: String -> String -> MathExpr
symbolMathExpr id name = Div (Plus [Term 1 [(Symbol id name [], 1)]]) (Plus [Term 1 []])

getSymId :: MathExpr -> String
getSymId (Div (Plus [Term 1 [(Symbol id name [], 1)]]) (Plus [Term 1 []])) = id

getSymName :: MathExpr -> String
getSymName (Div (Plus [Term 1 [(Symbol id name [], 1)]]) (Plus [Term 1 []])) = name
