...
デフォルトでは、コードのコピー＆ペースト時に元の書式を保持するため、自動インデントは**無効**になっています。もし自動インデントを有効にしたい場合は、Cursor/VS Codeの設定に以下を追加してください：
...
利用可能なオプション：
- `"none"`: 自動インデントなし（デフォルト）
- `"keep"`: 前の行のインデントを保持
- `"brackets"`: 括弧に基づく自動インデント
- `"advanced"`: 言語を考慮したインデント
- `"full"`: 完全な自動インデント
